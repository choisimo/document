...
**동기화 도구(synchronization primitive)**는 여러 실행 흐름이 공유 상태에 접근하는 순서와 동시 실행 가능 구간을 제한하는 메커니즘입니다. 도구를 사용했다는 사실만으로 데이터 일관성이 보장되지는 않습니다. 보호할 불변식, 임계 구역의 범위, 메모리 가시성 규칙, 실패 시 락 해제 경로가 함께 맞아야 합니다.

### 문서 범위와 학습 완료 기준

이 장의 예시는 운영체제 교과서의 공유 메모리 모델을 기준으로 합니다. 특정 언어의 메모리 모델이나 라이브러리 API를 구현 지침으로 대신하지 않습니다.

1. 공유 상태에서 반드시 유지할 불변식을 한 문장으로 정의할 수 있습니다.
2. 상호 배제, 진행, 유한 대기가 서로 다른 요구임을 반례로 설명할 수 있습니다.
3. 뮤텍스, 세마포어, 조건 변수, 모니터의 소유권과 대기 규칙을 비교할 수 있습니다.
4. 정상 경로뿐 아니라 취소, 예외, 시간 초과 때 락과 자원이 해제되는지 검토할 수 있습니다.
5. 예제의 전제와 실제 언어·하드웨어에서 추가로 필요한 원자성·메모리 순서 조건을 구분할 수 있습니다.
...
프로세스나 스레드의 명령은 서로 끼어들어 실행될 수 있습니다. 여러 실행 흐름이 동기화 없이 같은 가변 상태에 접근하고 그중 하나 이상이 값을 쓴다면, 관찰 결과가 실행 순서에 따라 달라질 수 있습니다. 실제 결과는 언어의 메모리 모델과 사용한 원자 연산에도 좌우됩니다.
...
**이 인터리빙의 결과:** `counter = 4`입니다. 두 연산이 직렬로 한 번씩 실행됐다고 가정한 기대값 `5`와 다릅니다. 아래 C 코드는 인터리빙을 설명하기 위한 의사 코드이며, 원자성이 없는 공유 변수 접근을 그대로 제품 코드에 사용해서는 안 됩니다.
...
3. **유한 대기 (Bounded Waiting)**: 프로세스가 임계 구역에 들어가기 위한 요청을 한 후 그 요청이 허가되기까지 다른 프로세스가 자신의 임계 구역에 들어갈 수 있는 횟수에는 상한(bound)이 존재한다.
