...
**주 메모리 관리**는 운영체제와 하드웨어가 가상 주소를 물리 저장 위치에 대응시키고, 페이지의 배치·보호·공유·회수를 제어하는 기능입니다. 보호와 효율은 단일 기법의 고정된 결과가 아니라 페이지 테이블 권한, 변환 캐시, 할당 정책, 워크로드를 함께 평가해 판단합니다.

### 문서 범위와 학습 완료 기준

이 장은 주소 바인딩, 연속 할당, 페이징, 페이지 테이블, 스와핑의 기본 모델을 다룹니다. 도표의 크기와 지연 시간은 별도 출처가 없는 경우 개념 계산을 위한 가정이며, 현재 CPU나 Linux 커널의 고정 사양을 뜻하지 않습니다.

1. 논리 주소, 가상 주소, 물리 주소가 문맥에서 무엇을 가리키는지 구분합니다.
2. 페이지 번호와 오프셋을 계산하고, 페이지 크기와 주소 폭을 계산 전제로 명시합니다.
3. 내부 단편화와 외부 단편화가 발생하는 할당 단위를 구분합니다.
4. TLB 적중과 실패 경로에서 필요한 메모리 접근 횟수를 가정과 함께 계산합니다.
5. 보호 비트, 사용자·커널 권한, 공유 매핑이 제공하는 경계와 남은 위험을 설명합니다.
...
메모리 보호 메커니즘은 프로세스의 접근을 허가된 주소와 권한으로 제한합니다. 실제 보장은 페이지 테이블 또는 베이스·리밋 설정이 올바르고, 권한 변경 경로와 커널 코드가 그 경계를 지킨다는 전제에 의존합니다.
