...
> **범위:** 이 문서는 POSIX `fork()` 의미와 현대 Unix 계열 커널에서 흔히 사용하는 copy-on-write 전략을 설명한다. Copy-on-write는 구현 전략이며 프로그램이 의존해야 할 API 계약은 아니다.
...
이 예제에서 부모와 자식은 처음에 `x = 5`를 관찰한다. 각 프로세스는 독립적인 가상 주소 공간을 가지므로 출력되는 가상 주소가 같아도 같은 물리 페이지가 계속 매핑된다는 뜻은 아니다. 자식이 `x = 10`, 부모가 `x = 20`을 기록하면 각 프로세스는 독립적인 값을 관찰한다. 부모와 자식의 출력 순서는 스케줄링에 따라 달라질 수 있다.

## 핵심 정리

`fork()`는 논리적으로 분리된 주소 공간을 가진 자식을 만들지만, 열린 파일 기술(open file description) 등 일부 자원은 POSIX 규칙에 따라 공유한다.
