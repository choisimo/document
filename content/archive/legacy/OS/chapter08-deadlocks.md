...
**교착 상태(deadlock)**는 실행 흐름 집합의 각 구성원이 집합 안의 다른 구성원만 일으킬 수 있는 사건을 기다려 더 이상 진행하지 못하는 상태입니다. 영향 범위는 교착에 참여한 스레드와 그 자원에 의존하는 요청으로 한정될 수도 있고, 핵심 자원이 묶이면 서비스 전체로 확장될 수도 있습니다.

### 문서 범위와 판정 기준

이 장은 재사용 가능한 자원을 요청·보유·해제하는 고전적 모델을 중심으로 설명합니다. 분산 시스템의 메시지 대기, 데이터베이스의 트랜잭션 교착, 언어 런타임의 비동기 태스크는 탐지 정보와 복구 주체가 다를 수 있습니다.

- 네 가지 필요조건은 교착 가능성을 설명하지만, 조건을 나열한 것만으로 현재 교착을 확정하지 않습니다.
- 자원 유형마다 인스턴스가 하나인 그래프에서는 순환이 교착을 뜻합니다. 인스턴스가 여러 개이면 순환만으로 충분하지 않을 수 있습니다.
- 시간 초과는 무한 대기를 끊는 정책이지 공유 상태의 원자적 복구를 자동으로 보장하는 해결책은 아닙니다.
- 진단 완료에는 대기 주체, 보유 자원, 요청 자원, 대기 시작 시각을 같은 시점의 증거로 연결한 자료가 필요합니다.
...
- 대기 그래프에서 필요조건과 실제 순환 대기를 구분합니다.
- 예방, 회피, 탐지·복구 전략이 요구하는 사전 정보와 비용을 비교합니다.
- 은행가 알고리즘의 최대 요구량 전제가 성립하는 범위를 설명합니다.
- 복구 대상을 고를 때 우선순위, 수행 비용, 롤백 가능성, 기아 위험을 함께 평가합니다.
...
이 코드는 두 스레드가 반대 순서로 락을 획득하도록 유도하는 학습 예제입니다. `sleep(1)`은 교착을 확정하는 동기화 장치가 아니므로 실행 시점에 따라 재현되지 않을 수 있습니다. 운영 코드에서는 전역 락 순서, 범위 기반 해제, 제한 시간과 실패 처리 정책을 함께 설계합니다.
...
교착 상태가 발생하려면 다음 네 가지 조건이 동시에 만족된다.

```mermaid
graph TD
    A[교착 상태<br/>4가지 필요 조건] --> B[상호 배제<br/>Mutual Exclusion]
