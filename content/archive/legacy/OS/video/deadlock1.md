# 🔒 교착상태(Deadlock) 학습 가이드

_필요조건, 예방, 회피, 탐지, 복구를 자원 상태 중심으로 구분하기_

> **범위와 근거:** 이 문서는 고전적인 단일 시스템 자원 할당 모델을 설명합니다. Python 코드는 알고리즘 구조를 보여주는 의사 코드이며, 가중치·백분율·실행 시간 표는 재현 절차와 출처가 없는 예시 값입니다. `example.com` 링크도 실제 학습 도구가 아닌 자리표시자이므로 사실 근거로 인용하지 않습니다.
...
> 교착상태 분석은 시스템 구성과 자원 사용 흐름을 기준으로 한다.
...
> 교착상태에 빠진 자원 집합에서는 네 가지 필요조건이 모두 성립합니다. 설계 단계에서 이 모델의 조건 하나를 지속적으로 깨뜨리면 해당 자원 집합의 교착상태를 예방할 수 있습니다.
...
| **🔮 회피**      | • 안전 상태를 유지하며 동적 할당    | • 최대 요구량 등 사전 정보 필요<br/>• 상태 검사 비용 | • 자원 요구량을 신뢰성 있게 예측 가능 |
...
아래 도식은 제품별 실제 구현을 증명하는 목록이 아니라 전략의 차이를 설명하기 위한 개념 예시입니다. 특정 운영체제나 클라우드가 어떤 전략을 쓰는지는 해당 구현 문서로 확인해야 합니다.
...
> 실제 시스템에서는 여러 전략을 조합한 하이브리드 접근법을 사용한다.
...
- **상호 배제(Mutual Exclusion)**: 공유 가능한 자원(예: 읽기 전용 파일)에는 상호 배제가 필요하지 않지만, 공유 불가능한 자원(non-sharable resources)에는 적용되므로 이 조건을 제거하기는 어렵습니다.
...
- **안전 상태(Safe State)**:
  ...
  - 안전 상태이면 교착상태가 발생하지 않습니다. 그러나 불안전 상태(unsafe state)가 곧 교착상태를 뜻하지는 않으며, 교착상태 발생 가능성이 있는 상태를 뜻한다.
...
- **자원 유형당 인스턴스가 하나인 경우(Single instance of a resource type)**: 자원 할당 그래프(Resource-Allocation Graph) 알고리즘을 사용합니다.
  - 클레임 간선(Claim edge): 프로세스 Ti가 자원 Rj를 요청할 수 있음을 나타내는 점선 화살표(Ti --> Rj)입니다. 프로세스 실행 전에 모든 클레임 간선이 그래프에 표시된다.
...
- **자원 유형당 인스턴스가 여러 개인 경우(Multiple instances of a resource type)**: **은행원 알고리즘(Banker's Algorithm)**을 사용합니다.
  - 각 스레드는 사전에 자신이 필요로 할 **최대 자원 사용량(maximum use)**을 선언한다.
  - 스레드가 자원을 요청할 때 대기해야 할 수도 있습니다.
  - 모든 자원을 얻은 스레드는 유한한 시간 내에 자원을 반납한다.
  ...
  - 자원 요청 알고리즘 (Resource-Request Algorithm for Process Pi): 프로세스 Pi가 자원 유형 Rj의 k개 인스턴스를 요청할 때:
    ...
    2. 만약 Request_i <= Available이면 3단계로 이동합니다. 그렇지 않으면, 자원이 사용 가능하지 않으므로 Ti는 대기한다.
...
> **근거 제한:** 아래 값에는 CPU 세대, 운영체제, 구현, 입력 그래프 밀도, 반복 횟수와 원시 결과가 없습니다. 재현 가능한 측정 데이터가 아니라 표 구성 예시로만 사용합니다.
...
- [동시성 프로그래밍 문서](https://example.com/concurrency-guide)
...
**학습 완료 기준**: 자원 상태에서 네 가지 필요조건을 식별하고, 안전 상태와 교착상태를 구분하며, 선택한 예방·회피·탐지 전략의 전제와 복구 비용을 설명할 수 있어야 합니다.

교착상태는 자원 획득 순서, 최대 자원 요구량, 탐지 주기, 복구 비용을 함께 고려하여 관리한다.
