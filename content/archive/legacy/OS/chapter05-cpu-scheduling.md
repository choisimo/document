# CPU 스케줄링: 정책, 지표, 적용 조건

_실행 가능한 작업 가운데 다음 작업을 선택하는 정책과 그 평가 방법_

## 문서 범위와 읽는 기준

이 문서는 단일 CPU의 기본 스케줄링에서 시작해 다중처리기와 실시간 스케줄링의 차이를 설명하는 학습 자료입니다. 수치가 포함된 계산 예시는 특정 운영체제의 측정 결과가 아니라 알고리즘 비교를 위한 가정입니다.

- **정책**은 다음에 실행할 작업을 고르는 규칙입니다.
- **메커니즘**은 타이머 인터럽트, 준비 큐, 컨텍스트 전환처럼 정책을 실행하는 수단입니다.
- **평가 기준**은 처리량, 대기 시간, 응답 시간, 마감 준수율처럼 워크로드에서 측정할 값입니다.
- 하나의 알고리즘이 모든 워크로드에서 우월하다고 가정하지 않습니다. 대화형 응답성, 배치 처리량, 공정성, 마감 시간 중 무엇을 우선하는지 먼저 정해야 합니다.
...
> **CPU 스케줄링**은 준비 상태의 스레드 또는 프로세스 가운데 CPU를 할당할 대상을 선택하는 운영체제 정책입니다. 효율성과 공정성은 자동으로 보장되는 속성이 아니라, 선택한 정책과 워크로드를 지표로 평가해 판단할 결과입니다.
...
| 영역 | 학습 결과 | 확인 방법 |
| --- | --- | --- |
| **기본 원리** | 준비·실행·대기 상태 전환에서 스케줄링 시점을 식별한다. | 상태 전이 예제에서 선점 여부를 설명한다. |
| **알고리즘** | FCFS, SJF, RR, 우선순위 정책의 선택 기준과 기아 위험을 비교한다. | 같은 작업 집합의 간트 차트와 대기 시간을 계산한다. |
| **성능 평가** | 평균값뿐 아니라 분산, 꼬리 지연, 처리량, 공정성을 구분한다. | 워크로드와 측정 구간을 명시한 지표표를 작성한다. |
| **적용 범위** | 범용, 다중처리기, 실시간 스케줄링의 목표 차이를 설명한다. | 각 환경에서 지켜야 할 제약과 포기할 지표를 짝지어 제시한다. |
...
스케줄링 정책은 워크로드가 체감하는 지연과 시스템이 완료하는 작업량의 분포를 바꿉니다. 효과를 판단할 때는 근거 없는 개선율 대신 동일한 하드웨어, 작업 집합, 측정 구간에서 다음 값을 비교합니다.

| 지표 | 측정 단위 | 해석할 때 함께 고정할 조건 |
| --- | --- | --- |
| CPU 활용률 | 측정 구간 중 비유휴 시간의 비율 | I/O 대기와 전력 절감용 유휴 상태 포함 여부 |
| 처리량 | 단위 시간당 완료 작업 수 | 작업 크기와 도착률 |
| 반환 시간 | 도착부터 완료까지의 시간 | 배치·대화형 작업 구분 |
| 응답 시간 | 도착부터 첫 응답까지의 시간 | 응답의 정의와 타임 슬라이스 |
| 공정성 | CPU 배분의 편차 또는 기아 발생 여부 | 우선순위와 관찰 구간 |

정책 변경의 완료 조건은 평균값 하나의 개선이 아닙니다. 우선 지표가 나아지고, 기아·마감 위반·컨텍스트 전환 비용 같은 가드 지표가 허용 범위를 벗어나지 않아야 합니다.
