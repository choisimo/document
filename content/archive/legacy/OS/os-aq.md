...
> **범위와 답안 기준:** 문항은 Linux/POSIX 환경의 파일 디스크립터, 프로세스, 시그널, IPC를 다룹니다. POSIX 규약과 Linux 전용 API를 구분하고, 구현·커널 버전에 따라 달라지는 성능 특성은 절대값으로 채점하지 않습니다. 코드형 답안은 반환값, `errno`, 부분 I/O, 사용하지 않는 파일 디스크립터의 닫기까지 설명해야 완결된 것으로 봅니다.

이 문제집은 파일 디스크립터 기초부터 `fork()`·`exec*()`, 파이프, 시그널, I/O 다중화까지 50개 문항으로 구성됩니다. 각 답안은 호출의 관찰 가능한 의미와 대표적인 실패 조건을 함께 제시합니다.
...
1. 답안: 파일 디스크립터는 리눅스 또는 유닉스 계열 시스템에서 프로세스가 파일을 다룰 때 사용하는 정수값이다. 프로세스가 파일을 `open()`하면 커널은 해당 프로세스의 파일 디스크립터 숫자 중 사용하지 않는 가장 작은 값을 할당한다[1][2].
...
6. 답안: signalfd() 함수의 주요 목적은 시그널을 받기 위한 파일 디스크립터 생성이다. 이를 통해 시그널 핸들러나 sigwaitinfo(2)가 아니라 select(2), poll(2), epoll(7) 같은 파일 디스크립터 기반 API로 시그널을 처리할 수 있다[3].
...
13. 답안: pipe() 함수는 파이프를 생성하며, 정수 배열을 인자로 받습니다. 성공 시 0을 반환하고, 실패 시 -1을 반환합니다. 파이프 생성 시 두 개의 파일 디스크립터가 생성되는데, `pipefd[0]`은 읽기 끝이고 `pipefd[1]`은 쓰기 끝입니다.
...
18. 답안: 파일 디스크립터 플래그와 파일 상태 플래그의 차이점은 다음과 같습니다:
   - 파일 디스크립터 플래그: 특정 파일 디스크립터와 관련된 속성으로, fcntl() 함수의 F_GETFD와 F_SETFD 명령으로 관리됩니다. 대표적인 예로 FD_CLOEXEC(exec 호출 시 파일 디스크립터 자동 닫기) 플래그가 있습니다. `fork()`로 생성된 자식은 해당 설정을 상속하지만, 이후 부모와 자식은 각자의 디스크립터 플래그를 독립적으로 변경할 수 있습니다[3].
...
20. 답안: 자식 프로세스가 종료되고 부모 프로세스가 자식의 종료 상태를 확인(`wait()` 또는 `waitpid()` 호출)하지 않으면 자식은 좀비 프로세스가 됩니다. 실행 메모리는 반환되지만 부모가 회수할 때까지 PID와 종료 상태를 담은 커널 항목이 남습니다.
...
24. 답안: 소켓은 네트워크 상의 두 프로그램 간 양방향 통신을 위한 엔드포인트입니다. 리눅스에서 소켓은 네트워크 통신을 위한 인터페이스를 제공하며, 해당 식별자(소켓 디스크립터)를 가진 열린 파일로 간주됩니다. 애플리케이션의 입장에서 소켓은 네트워크에서 파일을 읽고 쓰는 인터페이스를 제공하는 파일 디스크립터입니다[1].
...
27. 답안: epoll은 리눅스에서 제공하는 I/O 이벤트 알림 메커니즘으로, 대량의 파일 디스크립터를 효율적으로 처리할 수 있습니다. select/poll과 비교한 장점은 다음과 같습니다:
   - 성능: 매 호출마다 전체 감시 집합을 다시 전달하는 `select()`/`poll()`과 달리, 관심 집합을 커널에 등록하고 준비된 이벤트를 돌려받습니다. 실제 비용은 준비된 이벤트 수, 등록 변경, 커널 구현에 따라 달라집니다.
...
29. 답안: SA_RESTART는 `sigaction()`에서 지정하며, 커널이 재시작 가능하다고 정의한 일부 중단된 호출을 핸들러 반환 뒤 자동으로 재시작하게 합니다. 모든 시스템 콜에 적용되는 것은 아니므로 호출별 매뉴얼을 확인하고 `EINTR` 처리 경로를 유지해야 합니다.
...
36. 답안: SIGPIPE 시그널은 프로세스가 닫힌 파이프나 소켓에 쓰기를 시도할 때 발생합니다. 기본 처리는 프로세스를 종료하는 것입니다. 처리 방법은 다음과 같습니다:
   ...
   - 플랫폼 확인: `SO_NOSIGPIPE`는 BSD 계열에서 제공되는 옵션이며 Linux 소켓에는 일반적으로 없습니다. Linux에서는 `MSG_NOSIGNAL` 또는 프로세스의 `SIGPIPE` 처리 방식을 사용합니다.
...
38. 답안: vfork()는 fork()의 변형으로, 다음과 같은 차이점이 있습니다:
   ...
   - vfork()는 자식 프로세스가 `exec*()` 또는 `_exit()`를 호출하기 전까지 부모 프로세스를 정지시킵니다. 자식이 반환하거나 `exit()`를 호출하거나 허용되지 않은 메모리를 변경하는 코드는 피해야 합니다.
...
44. 답안: O_SYNC와 O_DSYNC는 모두 파일을 열 때 동기화 옵션을 지정하는 플래그이지만, 다음과 같은 차이가 있습니다:
   - O_SYNC: 파일 데이터와 메타데이터(inode 정보, 수정 시간 등)가 모두 디스크에 기록될 때까지 write() 호출이 반환되지 않습니다.
...
49. 답안: recvmmsg()와 sendmmsg() 함수는 다중 메시지를 한 번의 시스템 콜로 처리하기 위한 함수입니다:
   ...
   - 장점:
     - 시스템 콜 오버헤드 감소: 여러 recv/send 호출이 아니라 한 번의 호출로 처리
...
50. 답안: io_uring은 리눅스 커널 5.1부터 도입된 비동기 I/O 인터페이스로, 다음과 같은 특징과 장점이 있습니다:
   ...
   - 장점:
     - 비동기 I/O 인터페이스 통합: 파일과 소켓의 여러 작업을 같은 제출·완료 큐 모델로 다룹니다. 지원되는 연산과 비동기 실행 방식은 커널 버전, 파일시스템, 장치에 따라 다릅니다.
...
이 문제집의 학습 완료 기준은 파일 디스크립터 번호와 열린 파일 상태를 구분하고, `fork()`와 `exec*()`의 PID 의미를 설명하며, 파이프 EOF·`SIGPIPE`·`EINTR` 처리 경로를 코드에서 식별하는 것입니다. 성능 관련 답안은 API 이름만 나열하지 않고 워크로드와 측정 조건을 함께 제시해야 합니다.
