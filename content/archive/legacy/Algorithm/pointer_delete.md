...
포인터 해제 동작은 할당 방식, 객체 소유권, 배열 여부에 따라 달라진다. 예제를 적용하기 전에 `new`와 `delete`, `new[]`와 `delete[]`, `malloc`과 `free`의 대응 관계를 확인하고, 해제 뒤 재사용·중복 해제·소유권 불명확 상태를 별도의 실패 조건으로 구분한다. 컴파일 성공만으로 메모리 안전성이 검증됐다고 판정하지 않는다.

C++에서 `delete p`는 포인터 변수 자체를 삭제하지 않고, 포인터가 가리키는 힙 메모리 영역을 해제한다.
...
이러한 방식의 메모리 해제는 C++에서 동적으로 할당된 메모리를 관리하는 일반적인 패턴입니다. `new` 연산자로 할당한 메모리는 대응되는 `delete`로 해제해야 메모리 누수(memory leak)를 방지할 수 있습니다.

핵심은 `delete p`가 포인터 `p` 자체가 아니라 `p`가 가리키는 힙 메모리 영역을 해제한다는 점이다.
