...
첫 번째 일정안의 `O주`는 기간이 정해지지 않은 자리표시자이며 실행 가능한 일정이 아닙니다. 아래 순차 개발안과 뒤의 Agile 일정안은 서로 다른 계획 대안입니다. 팀 가용 인원, 시작일, 의존성, MVP 범위를 확정한 뒤 하나를 기준 일정으로 선택합니다.
...
아래 18주 구성은 기준안 후보입니다. 앞 장에서 선택 기능으로 분류한 커뮤니티와 전문가 연결은 별도 승인 없이 Sprint 7-8의 확정 범위로 간주하지 않습니다.
...
- **스프린트 완료**: 승인된 인수 조건, 코드 리뷰, 자동화 테스트, API·데이터 변경 문서가 모두 연결된 항목만 완료 처리합니다.
- **베타 진입**: 로그인, 일기 저장, 분석 실패 복구, 데이터 삭제 흐름을 통합 환경에서 확인하고 중대 개인정보·안전 결함이 없어야 합니다.
- **정식 릴리스**: 베타 피드백의 처리 결정, 모니터링·알람, 장애 대응 책임자, 롤백 절차를 기록해야 합니다.
- **일정 갱신**: 각 단계의 실제 시작일·종료일과 지연 사유를 계획 기간과 분리해 기록합니다.
