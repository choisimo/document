...
> **문서 구조:** 앞부분은 FUSE 옵션 참고 자료이고, 뒷부분은 `rasp`·`nano_pi`·`jcloud` 호스트의 SSHFS 장애 기록입니다. 호스트명, 사용자, 키 경로, 원격 경로는 당시 환경의 식별자이므로 현재 시스템의 `ssh -G <호스트>`, `findmnt`, systemd 로그로 다시 확인합니다.

SSHFS 문제는 SSH 연결, 원격 경로·권한, 수동 FUSE 마운트, systemd automount 순서로 범위를 좁힙니다. 앞 단계가 실패한 상태에서 다음 구성으로 넘어가지 않습니다.
...
2. **allow_other**
   ...
   - 사용 이유: 여러 로컬 사용자가 동일한 마운트 지점을 공유해야 할 때만 사용합니다. 비특권 사용자가 지정하려면 `/etc/fuse.conf`의 `user_allow_other`가 필요하며, 원격 권한과 로컬 `default_permissions` 검사를 함께 확인합니다[3].
...
이 절의 관찰 상태는 automount 유닛이 활성화되고 접근 요청은 감지하지만 실제 mount 유닛이 실패하는 경우입니다. `systemd-1 ... type autofs`는 트리거가 준비되었다는 증거일 뿐 SSHFS 연결 성공을 뜻하지 않습니다.
