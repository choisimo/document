...
Proxmox VE에서는 백업 완료/실패, 디스크 상태, 시스템 이벤트 등에 대한 알림을 이메일로 받을 수 있다. 이 문서는 Proxmox에서 이메일 알림을 설정하는 방법을 단계별로 정리한다.

## 개요 및 중요성

Proxmox의 이메일 알림 기능은 백업 상태, 디스크 상태, 시스템 이벤트 등을 실시간으로 모니터링할 수 있게 해주는 기능이다. 이 기능을 활성화하면 시스템 문제를 즉시 파악하고 대응할 수 있다.

## Gmail을 이용한 이메일 알림 설정

Gmail을 기준으로 설정 방법을 설명한다. 다른 이메일 서비스도 유사한 방법으로 설정 가능하다.

### 1. Gmail 앱 비밀번호 생성

Gmail에서 2단계 인증을 사용 중인 경우, 앱 비밀번호를 생성한다.
...
이메일이 아니라 디스코드로 알림을 받는 대안 구성은 다음과 같다.
