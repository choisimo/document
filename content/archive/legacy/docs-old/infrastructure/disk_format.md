# 리눅스 파일 시스템 관리 명령어 문서

파일 시스템을 관리하고 디스크를 초기화하는 명령어를 정리한 문서다. 각 명령어는 디스크 파티션, RAID 배열, LVM 볼륨 관리에 사용된다.
...
- 전체 제거에는 슈퍼블록 제거도 포함됨 (`--zero-superblock`)
...
- 최신 시스템에서는 2TB 이상의 디스크에 parted나 gdisk 사용 검토
...
# Proxmox VM에서 LVM 볼륨 용량 확장 문서

Proxmox VM에서 논리 볼륨(LVM)의 용량을 확장하는 방법을 정리한다. 특히 sda3 파티션과 관련된 LVM 저장 공간을 남은 빈 저장소 용량으로 증가시키는 절차를 설명한다.
...
Proxmox 호스트에서 VM 디스크 크기를 먼저 확장한다.
...
`-r` 옵션을 사용하지 않았다면 파일 시스템을 수동으로 확장한다.
...
4. LVM 구성이 복잡한 경우 전체 프로세스 순서를 확인한다: 디스크 확장 → 파티션 확장 → PV 확장 → LV 확장 → 파일 시스템 확장.
