...
마운트가 성공적으로 이루어졌는지는 다음 명령으로 확인한다.
```
df -h
```

이 명령은 마운트된 모든 파일시스템을 보여주며, 원격 디렉토리가 목록에 표시되는지 확인할 수 있다[3][5].
...
오류 메시지 "fuse -o \ ~~ 알 수 없는 옵션입니다"는 `/etc/fstab` 파일의 구문 문제를 나타낸다. 주된 문제점은 다음과 같다.

1. **백슬래시(`\`) 사용**: fstab의 각 마운트 항목은 한 줄에 작성되어야 한다. 백슬래시를 사용한 줄바꿈은 쉘 스크립트에서는 작동하지만 fstab에서는 지원되지 않는다.
...
fstab이 아니라 systemd 마운트 유닛을 사용하는 방법도 있다.
