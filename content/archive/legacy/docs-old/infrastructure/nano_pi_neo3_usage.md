# NanoPi Neo3 우분투 설치 후 로그인 방법

NanoPi Neo3에 우분투를 설치한 후 로그인 문제가 발생하는 경우, OS 이미지 종류와 버전에 따라 기본 로그인 정보가 다를 수 있다.
...
SSH로 접속을 시도하는 경우, 장치가 네트워크에 제대로 연결되어 있는지 확인한다. 직접 이더넷 케이블로 컴퓨터와 연결한 경우, 양쪽의 IP 주소가 같은 서브넷에 있어야 한다.

### 3. 다양한 로그인 조합 시도

다음과 같은 일반적인 조합을 시도한다.
- root / fa
- root / 1234
- ubuntu / ubuntu
- dietpi / dietpi
...
모든 방법이 실패할 경우, SD 카드를 다시 포맷하고 이미지를 새로 설치하는 방법을 검토한다. FriendlyElec 공식 이미지나 최신 Armbian 이미지를 사용할 수 있다.
...
성공적으로 로그인한 후에는 다음과 같은 기본 설정을 수행한다.
...
로그인 문제가 계속된다면 사용한 이미지 종류와 시도한 로그인 정보를 함께 확인한다.

--

# NanoPi Neo3 우분투 고정 IP 설정: NetworkManager 활용

NanoPi Neo3에 우분투를 설치한 후 NetworkManager 서비스를 통해 고정 IP를 설정하는 방법을 정리한다. 이 문서는 2025년 4월 기준 네트워크 관리 방식을 반영하며, CLI와 TUI 환경에서의 설정 방법을 설명한다.
...
## 4. Netplan과의 연동 구조
NetworkManager를 렌더러로 사용할 경우 Netplan 설정은 단순히 NetworkManager에 구성을 위임한다. `/etc/netplan/` 디렉토리의 YAML 파일은 다음과 같은 최소 구성을 유지한다.
...
## 10. 유의점과 운영 기준
1. **DHCP 예약 충돌 방지**: 라우터 측에서 IP 주소 예약 설정 필요
2. **이중화 구성 시**: bonding이 아니라 teamd 사용 검토
...
이 문서의 방법론은 NanoPi Neo3의 하드웨어 특성과 우분투 24.04 LTS의 네트워크 스택을 고려한다. 실제 환경 적용 전 테스트 베드에서의 검증을 수행하고, 산업용 제어 환경에서는 네트워크 구성 변경 시 다운타임 계획을 수립한다.


--

`error in libcrypto` 오류가 발생하는 상황에서 `.pub` 키가 생성되었다면, 해당 공개 키 자체가 문제일 가능성은 낮다. 다만 다음 항목을 확인한다.

## **공개 키 사용 시 문제될 수 있는 점**
1. **잘못된 키 파일 참조**:
   - SSH 클라이언트가 `.pub` 파일(공개 키)을 잘못 참조하여 인증을 시도할 경우 오류가 발생한다. 공개 키는 서버의 `~/.ssh/authorized_keys`에 저장되어야 하며, 클라이언트는 **개인 키**를 사용해 인증한다.
...
- 개인 키 파일 형식은 PEM 또는 DER로 저장되어야 하며, 암호화된 개인 키를 사용하는 경우 비밀번호를 제공한다.
...
`.pub` 키 자체는 문제가 없더라도, 이와 연관된 개인 키 또는 환경 설정 문제로 인해 인증 실패가 발생할 수 있다. 위 단계로 원인을 분리할 수 있다.
