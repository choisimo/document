...
IP 직접 접근을 차단하려면 다음과 같이 설정을 수정한다.
...
2. **SSL 핸드셰이크 문제**: IP 직접 접근 시 SSL 핸드셰이크는 서버 블록 결정 이전에 발생하므로, 연결 자체를 모두 거부하기는 어렵습니다[2]
...
IP 직접 접근 차단은 Cloudflare Zero Trust의 Access 인증 우회를 방지하는 중요한 첫 단계이지만, 전체 보안 경계로 보기에는 충분하지 않습니다.
...
2. **Bypass 정책의 위험성**: Cloudflare Zero Trust 환경에서는 Bypass를 사용해 내부 애플리케이션에 영구 접근 권한을 부여하는 방식이 보안 위험을 만들 수 있습니다[5].

3. **종합적 접근 방법**: 내부 네트워크 접근에는 Cloudflare Tunnel로 프라이빗 네트워크를 연결하고 사용자가 WARP를 통해 접속하는 구성을 사용할 수 있습니다[5].
...
2. **SSL 인증서 경로 확인**:
   ...
   ```
   인증서 파일이 존재하는지, 권한이 올바른지 확인한다.

3. **단계별 테스트**:
   설정 파일에 문제가 있다면, 간단한 버전부터 시작하여 점진적으로 복잡하게 만들어 어떤 부분에서 오류가 발생하는지 확인한다.

4. **SSL 모듈 확인**:
   ```bash
   nginx -V | grep ssl
   ```
   Nginx가 SSL 지원과 함께 컴파일되었는지 확인한다.

5. **`ssl on` 지시어 제거**:
   이 지시어는 최신 버전에서 사용되지 않으며, `listen 443 ssl`로 대체되었습니다.
...
이미 `default_server` 지시어가 설정된 서버 블록이 있다면 동일한 포트에 두 번째 `default_server`를 추가할 때 "Duplicate Default Server" 오류가 발생합니다[7].

Nginx는 같은 IP 주소와 포트 조합에 대해 **오직 하나의** `default_server` 지시어만 허용합니다[7]. 해당 설정에는 포트 80에 대해 두 개의 `default_server` 지시어가 있습니다:
...
1. **기존 default 설정 수정**: 기존 default 서버 블록을 IP 접근 차단용으로 변경한다[2]:
   ```bash
   sudo nano /etc/nginx/sites-enabled/default
   ```
   
   기존 내용을 IP 차단 설정으로 교체하거나 수정한다[6].

2. **중복 제거**: 추가하려는 설정이 이미 동일한 기능을 하는 경우, 중복을 피하고 기존 설정만 사용한다[2][7].

3. **설정 후 테스트**: 변경 후 구성을 테스트한다[7]:
   ```bash
   sudo nginx -t
   ```

4. **HTTPS 차단 참고사항**: HTTPS에 대한 IP 접근 차단을 위해 Nginx 1.19.4 이상에서는 `ssl_reject_handshake on;` 사용을 검토한다. 더 낮은 버전에서는 403 반환 방식이 적합합니다[3][6].
