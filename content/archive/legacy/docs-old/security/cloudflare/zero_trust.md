...
> **보존 상태:** 이 파일은 여러 시점의 설치 절차, 장애 분석, 검색 메모가 합쳐진 레거시 문서이며 일부 코드 블록과 인용 식별자가 손상되어 있습니다. 현재 Cloudflare·Docker·Nginx 설치 문서로 실행하지 말고, 설계 후보와 과거 장애 가설을 찾는 자료로만 사용합니다.

## 적용 전에 분리할 경계

- **정본 버전**: Cloudflare Tunnel/Access, cloudflared, Docker, Nginx Proxy Manager의 정확한 버전과 공식 설정 스키마
- **인증 경계**: 공개 DNS, Cloudflare Access 정책, 터널 자격 증명, 원본 서비스 인증 중 각 단계의 책임
- **비밀 경계**: 터널 토큰, API 토큰, 세션 키를 파일·명령 기록·이미지에 남기지 않는 공급 방식
- **완료 증거**: 허용 사용자 성공, 미인증·비허용 사용자 거부, 원본 IP 직접 접근 거부, 재시작 후 정책 유지, 감사 로그의 동일 요청 ID

표의 `높음·중간·낮음` 보안 수준과 비용은 평가 기준이나 조사 시점이 없어 비교 결과로 사용할 수 없습니다. 현재 요구사항을 기준으로 공격면, 운영 주체, 장애 의존성, 비용을 다시 측정합니다.
