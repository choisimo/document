...
이 문서의 표본은 MARK 규칙 추가 실패를 기록하지만 원인과 재현 범위는 환경별 확인이 필요합니다. 이 보고서에서는 오류의 원인을 자세히 분석하고, 다양한 해결 방법을 비교하여 최적의 접근법을 제시합니다.

## 문서 상태와 진단 가드

이 보관 문서는 동일 문단이 반복되고 코드 펜스와 명령 문자열 일부가 손상되어 있어 그대로 실행할 수 있는 런북이 아니다. 표본 로그가 직접 보여 주는 사실은 `ip6tables`의 MARK 규칙 추가가 실패했다는 점까지이며, 커널 모듈 부재·nftables 전환·Tailscale 결함은 확인이 필요한 가설이다. 조치 전 배포판, 커널, Tailscale, iptables backend, `lsmod`·`modinfo` 결과를 기록한다. 방화벽 flush 명령은 기존 접근 제어와 NAT를 제거할 수 있으므로 실행하지 않는다. 커널 변경은 현재 부팅 항목과 복귀 가능한 이전 커널을 확인한 뒤 진행하고, 완료는 Tailscale 상태·IPv4/IPv6·서브넷 경로·MagicDNS를 각각 재검증해 판정한다.
...
### 2. 커널 업데이트 적용
**효과와 작동 원리**:
- Fedora 41·커널 6.11.6 조합에서 해결됐다는 보고가 있으나 현재 시스템에도 같은 결과가 나는지는 재검증해야 합니다.
