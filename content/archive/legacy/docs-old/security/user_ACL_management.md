# 사용자 홈 디렉터리·SSH·PAM 관리 가이드

Linux 시스템에서 사용자별 기본 디렉토리(홈 디렉토리)를 설정하는 다양한 방법을 자세히 설명해 드리겠습니다.

## 변경 범위와 복구 조건

이 문서는 홈 디렉터리 관리와 SSH/PAM 보안 설정이라는 별도 작업을 함께 보관한다. 홈 이동 전 대상 사용자 프로세스 정지, 원본 백업, 소유권·ACL·확장 속성 보존, 대상 파일시스템 여유 공간을 확인하고 로그인·파일 접근을 검증한 뒤 원본을 정리한다. SSH/PAM 변경 전 현재 root 세션을 유지하고 설정 파일을 백업하며, `sshd` 구문 검사와 별도 세션 로그인이 성공할 때까지 기존 세션을 닫지 않는다. 배포판별 PAM 모듈과 OpenSSH 버전 차이는 확인되지 않은 전제로 남기고, 실패 시 복원할 파일과 명령을 먼저 기록한다.
...
홈 디렉터리를 생성하는 도구와 설정에 따라 `/etc/skel`의 내용이 새 홈에 복사될 수 있습니다:
