...
Docker 설치 후 기본적으로 **root** 사용자만 실행 권한을 가진다. Docker를 비관리자 사용자도 실행할 수 있도록 해당 사용자를 `docker` 그룹에 추가한다.
...
2. 명령 실행 후 변경 사항을 적용하려면 **로그아웃 후 다시 로그인**한다.
...
## 주의 사항
- **`sudo visudo`** 명령으로 수정할 때는 구문 오류를 피한다. 잘못된 구문이 있으면 사용자 권한이 손상될 수 있습니다.
- Docker 권한을 부여한 후에는 로그아웃/로그인 또는 `su - $USER` 명령을 통해 새로운 세션을 시작해야 권한이 적용됩니다.

--- 

위 내용을 기준으로 Docker 및 사용자 권한 설정을 관리한다.
