...
Docker의 `docker network create` 명령어는 컨테이너 간 통신 및 네트워크 격리를 제어하는 핵심 도구로, 다양한 옵션을 통해 네트워크 토폴로지를 세밀하게 구성할 수 있습니다[5][7]. 본 분석에서는 문서에 수록된 주요 옵션의 기능과 사용 사례, 적용 예시를 설명합니다.

---

## 옵션 범위와 명령 판정 기준

이 보관 문서는 여러 시점의 Docker 네트워크 설명을 합친 자료이므로 같은 옵션이 반복되며 지원 여부는 Docker Engine 버전, 드라이버, Swarm 상태에 따라 달라진다. `host`와 `none`은 기본 제공 네트워크 모드로 이해하고 일반 사용자 정의 네트워크 생성 대상과 구분한다. 코드 블록 끝의 `[숫자]` 표시는 출처 표식이지 명령 인수가 아니므로 터미널에 포함하지 않는다. 실행 전 `docker network create --help`, 기존 주소 대역과 네트워크 목록을 확인하고, 생성 뒤에는 `docker network inspect` 결과와 의도한 연결·차단 경로를 각각 검증한다.
...
### 3. `--gateway`
- **기능**: 서브넷의 게이트웨이 IP 지정
- **자동 할당**: 값을 생략하면 Docker가 서브넷에서 게이트웨이를 정하므로 실제 값은 `docker network inspect`로 확인
- **예시**:
  ```bash
...
- **none**: 컨테이너를 호스트 및 다른 컨테이너로부터 네트워크 격리합니다
...
**ipvlan**: IPv4 및 IPv6 주소 지정에 대한 세부 제어를 제공합니다[3].
