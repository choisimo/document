# VS Code Project Manager 활용 가이드

> **적용 범위:** 이 문서는 Project Manager 확장의 로컬 프로젝트 목록, 태그, 저장소 검색 설정을 설명합니다. 확장 게시자·Marketplace 식별자·버전이 원문에 없으므로 설정 키와 단축키는 설치된 확장의 명령 팔레트와 설정 화면에서 확인해야 합니다.

Project Manager는 자주 여는 폴더와 워크스페이스를 이름·태그로 저장해 전환 단계를 줄입니다. 일정, 리소스, 산출물을 관리하는 프로젝트 관리 시스템이나 VS Code 프로필 동기화 기능을 대신하지는 않습니다.
...
Project Manager를 사용하면 프로젝트 위치와 무관하게 즐겨찾기 목록을 정의하거나 Git, Mercurial, SVN 저장소를 자동으로 감지하도록 설정할 수 있다[4].
...
3. `.vscode/settings.json`을 저장소에 포함하면 VS Code가 해당 폴더를 열 때 워크스페이스 설정을 읽습니다[10]. 이 동작은 Project Manager와 독립적이며, 팀원이 파일을 받았는지와 필요한 확장을 설치했는지 별도로 확인해야 합니다.
...
Project Manager 태그는 개인의 탐색 보조 정보입니다. 팀 일정, 리소스, 산출물의 공식 상태는 공유 이슈 추적기나 프로젝트 관리 시스템에 기록하고, 이 확장의 태그를 근거 자료로 사용하지 않습니다[2][3].
...
개발자의 작업 환경과 프로젝트 특성에 맞게 Project Manager를 customizing하여 사용하면, 프로젝트 간 전환 시간을 줄이고 일관된 개발 환경을 유지하는 데 도움이 된다. 여러 프로젝트를 동시에 관리하는 개발자나 팀에서 특히 활용도가 높다.

태그 기반 분류, Git 저장소 자동 감지, 단축키 활용, VSCode 프로필 통합 등의 기능을 조합하면 작업 방식에 맞는 개발 환경을 구성할 수 있다.
