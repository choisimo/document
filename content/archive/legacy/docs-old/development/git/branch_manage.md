...
로컬 브랜치의 push 대상이 의도와 다르면 새 원격 브랜치가 생기거나 다른 브랜치를 갱신할 수 있습니다. 브랜치 이름 자체보다 upstream, refspec, `push.default`를 확인해 연결을 교정합니다.

## 브랜치 연결 판정

로컬·원격 브랜치 이름이 다르다는 사실만으로 매번 새 브랜치가 생성되지는 않는다. 실제 push 대상은 upstream, 명시한 refspec, `push.default` 설정이 함께 결정한다. 아래 설정을 모두 적용하지 말고 팀 정책에 맞춰 이름 통일, 명시적 upstream, 매회 refspec 중 하나를 선택한다. 명령 전후 `git branch -vv`와 설정 출처를 확인해 현재 로컬 브랜치가 어느 원격 ref를 추적하는지 판정한다. `--global`이 없는 `git config` 예시는 현재 저장소에만 적용된다.
...
원격 브랜치와 로컬 브랜치의 이름 불일치 문제는 위의 방법들로 해결할 수 있습니다. 일반적인 기준은 다음과 같다.
