...
Git에서 `pull` 명령어를 실행할 때 적용되는 세 가지 주요 설정 옵션과 동작 예시를 정리한다.

## 설정 범위와 선택 기준

세 설정은 서로 다른 pull 정책이며 동시에 권장되는 단계가 아니다. `--global`이 없는 예시는 현재 저장소 설정이고, 실제 적용값은 시스템·전역·저장소 설정의 우선순위에 따라 달라진다. 공유된 커밋을 rebase하면 커밋 ID가 바뀌므로 게시 여부를 먼저 확인한다. 선택 뒤에는 설정 출처, 현재 upstream, pull 전후 커밋 그래프를 확인해 merge commit 생성, 로컬 커밋 재작성, fast-forward 거부 중 어떤 상태가 발생했는지 판정한다.

## 1. `git config pull.rebase false` (Merge 방식)

이 설정은 pull 시 merge를 명시적으로 선택한다. 실제 기본값은 Git 버전과 기존 설정에 따라 달라질 수 있다.
...
git pull 실행 후:
A -- B -- C -- D' -- E' (local main; origin/main은 C에 유지)
```
...
| 옵션 | 장점 | 단점 | 적합한 상황 |
|------|------|------|------------|
| **Merge** | - 이력이 보존됨- 병합 시점이 명확함 | - 이력이 복잡해질 수 있음 | - 공개 브랜치 작업- 이력 보존이 중요할 때 |
