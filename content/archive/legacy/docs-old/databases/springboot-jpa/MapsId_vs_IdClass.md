# `@MapsId`, `@EmbeddedId`, `@IdClass` 선택 기준

이 문서는 연관 관계의 외래 키가 엔티티 기본 키의 전부 또는 일부이기도 한 매핑을 비교합니다. 선택은 성능 우열이 아니라 스키마 키 구조, 도메인에서 키를 값 객체로 다룰지 여부, 쿼리 경로, 기존 매핑과의 일관성으로 판정합니다.

특히 `@OneToOne`이나 `@ManyToOne` 관계에서 연관 엔티티의 기본 키를 참조 엔티티의 기본 키로 사용할 때 유용합니다.

## @MapsId의 명시적 선언이 필요한 이유

`@MapsId`는 연관 객체의 식별자를 현재 엔티티의 기본 키 속성에 대응시킨다는 계약을 명시합니다.

1. 외래 키와 기본 키를 동일하게 유지할 수 있음
2. 별도의 키 생성 전략이 필요하지 않음
3. 연관 객체와 키 열의 매핑 의도를 한 곳에 표현

`@MapsId` 자체가 조인 쿼리 성능을 높이거나 데이터 일관성을 완전히 보장하지는 않습니다. 실행 계획은 인덱스와 쿼리에 좌우되고, 일관성은 데이터베이스의 기본 키·외래 키 제약과 엔티티 생성 규칙이 함께 유지합니다.
...
3. **키를 하나의 값 객체로 전달할 때**
   - 여러 키 필드를 함께 비교·전달하는 도메인 API를 명확하게 표현할 수 있음
...
선택을 완료하려면 실제 스키마의 기본 키·외래 키 제약을 매핑하고, 키 생성·조회·연관 저장·삭제를 통합 테스트하며, 생성된 SQL과 실행 계획을 비교합니다. `@EmbeddedId`의 중첩 경로가 팀의 쿼리 관례에 맞는지, `@IdClass`의 중복 필드가 일관되게 유지되는지도 함께 검토합니다.
