...
이 문서는 세 개의 개선안과 후속 답변을 누적한 비교 자료이며 하나의 확정된 실행 스크립트가 아니다. 사용할 구현 하나를 선택한 뒤 Gemini API 버전, model ID, 요청·응답 스키마, temperature와 출력 token 제한을 현재 계약에 맞춰 고정한다. API 키를 명령행·URL·디버그 출력에 노출하지 않고, `sed`를 JSON parser의 안전한 대체물로 간주하지 않는다. HTTP 상태, API 오류 객체, 빈 candidate, 429·5xx 재시도와 비용 한도를 별도 상태로 처리한다. 완료는 고정된 fixture의 정상 응답, 잘못된 키, rate limit, 비정상 JSON, 중단 시 임시 파일 정리를 각각 확인해 판정한다.
