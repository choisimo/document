# 스케줄 관리 오픈소스 후보 비교 (출처 재검증 필요)

이 보관 보고서는 일반 사용자용 스케줄 관리 후보 5개를 Docker 배포 관점에서 비교한다. 조사 시점과 원자료 목록이 없어 GitHub 활동·커뮤니티 만족도 관련 수치는 현재 상태로 재검증해야 한다.

## 비교 시점과 추천 판정 기준

이 보관 보고서에는 조사일, GitHub 활동 수치의 조회 기준, Reddit 표본, 사용자 만족도 78점의 원문 출처가 포함돼 있지 않다. 해당 수치와 “공식 이미지”, 지원 버전, 번역률, 메모리 사용량은 검증되지 않은 주장으로 취급한다. `latest` 이미지와 예시 비밀번호를 운영 배포에 사용하지 않고, 후보별 고정 버전·라이선스·최근 release·백업·인증·캘린더 연동을 동일한 시험표로 비교한다. 최종 선택은 예약 대상, 사용자 수, 필수 통합, 운영 역량, 총비용의 우선순위를 먼저 정하고 실제 시험 결과로 판정한다.
...
## 결론
현재 자료만으로 단일 최적 후보나 고정 순위를 확정할 수 없다. 종합 예약, 단순 약속 관리, CalDAV 운영처럼 목표를 분리하고 동일한 버전·데이터·인증 조건에서 시험한 결과로 후보를 선택한다. 향후 과제로는 모바일 네이티브 앱 지원 강화와 AI 기반 스케줄 최적화 기능 추가가 필요할 것으로 보입니다.
