...
2. **세션 내에서 exit 명령 사용**: 세션을 분리(detach)하지 않고 세션 내에서 exit 명령을 실행하면 해당 세션이 종료됩니다[3]
...
### 1. 올바른 세션 분리 사용하기
터미널을 종료하기 전에 `Ctrl+b d`를 사용하여 세션을 분리한다[2][3]. 이렇게 하면 세션이 백그라운드에서 계속 실행됩니다.
...
1. **다른 사용자로 실행된 세션 확인**: 세션이 보이지 않는다면 다른 사용자 계정으로 실행되었을 수 있습니다[10]. 다른 사용자로 로그인하여 세션을 확인한다.
