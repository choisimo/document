...
`Start At`·`Run duration`의 UI와 계산 방식은 changedetection.io 버전과 서버 시간대에 따라 달라질 수 있으므로 문서의 예시를 현재 동작으로 단정하지 않는다. 서버·컨테이너 시간대와 자정을 넘는 구간, 요일 경계를 시험해 실제 실행 시각을 확인한다. Discord 웹훅 URL의 token은 그 자체가 메시지 전송 권한이므로 문서, 화면 캡처, 로그에 노출하지 않고 유출 시 즉시 폐기·재발급한다. 완료는 시험 변경 한 건에 대해 예정 시간대에 알림이 한 번 도착하고, 비활성 시간대에는 요청이 발생하지 않으며, 실패 로그와 재시도 상태를 확인한 경우로 한정한다.
...
4. **웹훅 URL 자체가 자격 증명**: 별도 로그인 없이 URL의 token으로 메시지를 보낼 수 있으므로 비밀값으로 저장하고 노출 시 재발급합니다.
## 네트워크 설정 요구사항
