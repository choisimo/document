...
이 보관 보고서는 서로 다른 시점의 n8n·MCP 설정안을 누적했으며 노드 수, 템플릿 수, MCP 지원 방식과 환경 변수는 n8n 버전에 따라 달라진다. 배포 전 n8n 이미지·커뮤니티 노드·MCP 클라이언트 버전을 고정하고 현재 공식 문서에서 노드 이름과 endpoint를 확인한다. 이메일 전송과 외부 API 호출은 테스트 요청도 실제 부작용을 만들 수 있으므로 인증, 호출 주체 권한, 입력 스키마, 대상 allowlist, 중복 요청 처리와 감사 로그를 먼저 구성한다. API 키는 워크플로·URL·로그에 직접 넣지 않는다. 완료는 인증 없는 요청 거부, 허용 도구만 노출, 성공·실패 응답, 재시작 후 워크플로 지속성을 증거로 판정한다.
...
초기 도입은 쉽지만, 다중 서버 구성이나 고급 통합에는 상당한 기술적 지식이 필요합니다. 설정 후에도 AI 모델이 호출할 수 있는 기능은 노출한 workflow와 권한 범위로 제한되며, 각 부작용은 별도의 승인·감사 계약이 필요합니다.
...
3. 설정을 구성합니다:
   ...
   - **인증**: 외부 연결이 차단된 로컬 시험에서만 "None"을 사용하고, 그 밖의 환경에는 API 키나 header 인증 적용
