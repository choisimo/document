...
이 문서는 데이터베이스 개념을 폭넓게 정리한 보관용 학습 자료다. SQL 문법, 기본 격리 수준, 잠금, 실행 계획, 성능 특성은 DBMS와 버전에 따라 달라질 수 있으므로 제품 공통 개념과 제품별 동작을 구분해 읽는다. 운영 결론은 예제 설명만으로 확정하지 않고 대상 DBMS의 공식 문서, 실행 계획, 실제 쿼리 결과를 근거로 판정한다.
