...
이 문서는 트랜잭션과 동시성 제어를 설명하는 보관용 학습 자료다. 자동 커밋, DDL의 암시적 커밋, 기본 격리 수준, 잠금 범위, 스토리지 엔진 동작은 DBMS와 버전에 따라 달라진다. 따라서 절대적 표현은 문서의 예시 시나리오에 한정해 해석하고, 실제 설계에서는 공식 문서, 스키마 제약, 동시 실행 테스트와 실패 시 롤백 결과를 함께 확인한다.
...
**🔍 문제:** 트랜잭션(Transaction)이 "프로그램 수행의 논리적인 단위"라고 불리는 이유를 은행 계좌 이체 예시로 설명한다.
...
**🔍 문제:** 트랜잭션의 ACID 속성 중, **원자성(Atomicity)**이 보장되지 않았을 때 발생할 수 있는 치명적인 문제점을 설명한다.

**💡 해설:**
- **원자성 원칙**: `All or Nothing` - 모든 연산이 성공하거나 모두 실패해야 함
- **위반 시 결과**: 일부 연산만 성공하는 **미완료 상태** 발생
...
**🔍 문제:** 일관성(Consistency) 속성의 주요 책임 주체(DBMS vs. 개발자)와 그 이유를 정리한다.
...
**🔍 문제:** **고립성(Isolation)**이 필요한 이유를 '더티 리드(Dirty Read)' 현상을 예로 들어 설명한다.
...
**🔍 문제:** **지속성(Durability)**을 보장하기 위해 DBMS가 사용하는 핵심 메커니즘을 정리한다.
...
**🔍 문제:** 트랜잭션의 5가지 상태(Active, Partially Committed, Committed, Failed, Aborted) 전이 과정을 설명한다.
...
**🔍 문제:** 데이터베이스에서 여러 트랜잭션을 순차적이 아닌 동시에 처리(Concurrency)하는 이유 2가지를 설명한다.
...
**🔍 문제:** 스케줄(Schedule)의 의미와 각 트랜잭션 내부 명령 순서를 보존해야 하는 이유를 정리한다.
...
**🎯 결론**: 트랜잭션 내부 순서는 **논리적 무결성**을 위해 보장되어야 함

---

### Q9. 직렬 가능한(Serializable) 스케줄

**🔍 문제:** '직렬 가능한(Serializable)' 스케줄이 동시성 제어에서 가지는 의미를 정리한다.
...
**🎯 결론**: 동시성의 이점을 유지하면서도 **데이터베이스 일관성을 강하게 보장**하는 스케줄

---

### Q10. 충돌(Conflict)의 3가지 조건

**🔍 문제:** 두 연산이 '충돌(Conflict)'한다고 판단할 수 있는 3가지 조건을 정리한다.
...
결과: T1 → T2 간선 생성
의미: 최종 직렬 스케줄에서 T1이 T2보다 먼저 와야 함
```

**🎯 결론**: 간선은 트랜잭션 간의 **실행 순서 의존성**을 표현

문제 13. 우선순위 그래프에 사이클(예: T1 → T2 → T1)이 존재하면 충돌 직렬성이 깨지는 이유를 설명한다.
...
문제 14. 스케줄의 '복구 가능성(Recoverability)'의 의미와 중요성을 정리한다.
답안: 복구 가능성이란, 트랜잭션 T_j가 T_i가 쓴 데이터를 읽었을 경우 T_i의 커밋(Commit)이 T_j의 커밋보다 먼저 일어나야 한다는 조건입니다. 이는 T_i가 롤백되었을 때 T_j가 존재하지 않는 데이터를 기반으로 커밋하는 '더러운 읽기' 문제를 방지하고, 데이터베이스가 항상 일관된 상태로 복구될 수 있도록 보장하기 때문에 중요합니다.

문제 15. '연쇄적 롤백(Cascading Rollback)'의 의미와 비효율성을 설명한다.
...
문제 16. '비연쇄인 스케줄(Cascadeless Schedule)'이 연쇄적 롤백 문제를 해결하는 방식을 정리한다.
...
답안: "어떤 트랜잭션이 다른 트랜잭션이 쓴 데이터를 읽으려면, 쓰는 트랜잭션이 커밋(Commit)을 완료한 이후에만 읽을 수 있다"는 더 엄격한 규칙을 적용합니다. 이 규칙을 통해, 아직 커밋되지 않은 데이터(롤백될 가능성이 있는 데이터)를 다른 트랜잭션이 읽지 못하도록 차단하므로, 연쇄적 롤백이 발생할 가능성 자체가 사라집니다.
...
**🔍 문제:** SQL에서 트랜잭션의 '고립성 수준(Isolation Level)'을 여러 단계로 제공하는 이유를 정리한다.
...
graph TD
    subgraph "고립성 수준별 특성"
        ...
        D[Serializable] --> D1[가장 강한 일관성<br/>성능 최저]
    end
...
**🔍 문제:** 가장 낮은 고립성 수준인 Read Uncommitted에서 발생하는 '더티 리드(Dirty Read)' 현상을 설명한다.
...
**🔍 문제:** Read Committed 수준에서 'Non-Repeatable Read' 현상이 발생하는 이유를 설명한다.
...
**🔍 문제:** Repeatable Read 수준이 'Non-Repeatable Read' 문제를 해결하는 방식을 정리한다.
...
**🎯 결론**: **스냅샷 격리**를 통해 트랜잭션 내 **읽기 일관성을 강하게 보장**

---

### Q21. 팬텀 리드(Phantom Read) 현상

**🔍 문제:** '팬텀 리드(Phantom Read)'의 의미와 Repeatable Read 수준에서도 이 현상이 발생할 수 있는 이유를 정리한다.
...
**🔍 문제:** MySQL(InnoDB 엔진)이 Repeatable Read 수준에서 갭 락(Gap Lock)과 넥스트 키 락(Next-Key Lock)을 사용하여 팬텀 리드를 방지하는 원리를 간략히 설명한다.
...
**🔍 문제:** 가장 높은 고립성 수준인 Serializable의 동시성 제어 방식과 장단점을 정리한다.
...
- **효과**: 다른 트랜잭션의 데이터 수정을 차단

**⚖️ 장단점 분석**

**✅ 장점**
```
강한 일관성: 모든 종류의 데이터 부정합 문제 방지
- ❌ Dirty Read 방지
...
**🎯 결론**: **강한 일관성 vs 심각한 성능 저하** - 매우 중요한 데이터에 제한적으로 사용
...
**🎯 결론**: 이 선택은 일반적인 출발점일 수 있지만, 허용할 이상 현상과 실제 읽기·쓰기 경합을 측정한 뒤 고립성 수준을 결정해야 한다.
...
문제 25. SQL에서 트랜잭션을 시작, 성공적으로 종료, 실패하여 취소하는 명령어를 정리한다.

---

### Q25. SQL 트랜잭션 제어 명령어

**🔍 문제:** SQL에서 트랜잭션을 시작, 성공적으로 종료, 실패하여 취소하는 명령어를 정리한다.
...
**🔍 문제:** DROP TABLE과 같은 DDL(데이터 정의어) 문장이 트랜잭션 컨텍스트에서 특별한 주의가 필요한 이유를 정리한다.
...
**🎯 결론**: DDL의 커밋 동작은 DBMS와 명령별로 다르다. 대상 DBMS의 공식 문서에서 암시적 커밋 여부를 확인하고 DML 트랜잭션과 분리한다.

---

### Q27. 동시 좌석 예매 시 트랜잭션 부재 문제

**🔍 문제:** 두 명의 사용자가 동시에 '공연장 좌석 A-1'을 예매하려고 할 때 트랜잭션이 없다면 발생할 수 있는 문제를 설명한다.
...
**🎯 결론**: 트랜잭션과 적절한 잠금은 이 예매 흐름의 중복 갱신 위험을 줄인다. 최종 유일성은 좌석 키의 `UNIQUE` 제약과 실패 시 `ROLLBACK` 결과까지 함께 검증해야 한다.

---

### Q28. 약한 일관성이 허용되는 트랜잭션 예시

**🔍 문제:** 약한 수준의 일관성(Weak Levels of Consistency)이 허용되는 트랜잭션 예시를 정리한다.
...
**🔍 문제:** MySQL의 InnoDB 스토리지 엔진은 트랜잭션을 지원하고 MyISAM 스토리지 엔진은 지원하지 않는다. 이 차이가 시스템 설계에 미치는 영향을 정리한다.
...
**🎯 결론**: 스토리지 엔진은 트랜잭션·동시성·복구·내구성 요구와 실제 워크로드 측정 결과로 선택한다. 단순 조회라는 이유만으로 MyISAM을 기본 선택하지 않는다.

---

### Q30. 트랜잭션 간 충돌 분석

**🔍 문제:** 항공기 좌석을 예약하는 트랜잭션 T1(좌석 상태 변경(write), 예약 내역 기록(write))과 예약 현황을 조회하는 트랜잭션 T2(좌석 상태 조회(read), 예약 내역 조회(read))가 동시에 실행될 때 두 트랜잭션 간 충돌 여부를 설명한다.
...
**⚖️ 동시성 제어 필요성**
- **데이터 일관성**: T2가 중간 상태의 미완료 데이터를 읽지 않도록 제어
- **실행 순서**: T1 완료 후 T2 실행 또는 적절한 고립성 수준 설정
- **비즈니스 요구**: 예약 과정 중 정확한 현황 조회 보장

**🎯 결론**: **Write-Read 충돌 발생** → **동시성 제어 메커니즘 필요**

---

## 🎓 요약

데이터베이스 트랜잭션과 동시성 제어에 대한 **30개 핵심 문제**를 정리했다.
...
이 내용은 **실제 시스템 설계와 데이터베이스 최적화**에서 트랜잭션 경계와 고립성 수준을 결정할 때 활용할 수 있다.
