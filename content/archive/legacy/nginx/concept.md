# Nginx 구성과 운영 패턴: 레거시 참고 자료

> **문서 상태:** 이 파일은 서로 다른 Nginx 배포판·에디션·시점의 예제와 생성된 설명을 합친 기록입니다. 원문에는 Markdown 구조가 없는 구간, 미해결 인용 표식, 버전 조건이 없는 성능·보안 평가가 포함돼 있습니다. 현재 서버 설정으로 복사하지 말고 지시어 후보를 찾는 참고 자료로만 사용합니다.

## 적용 원칙

1. `nginx -V`로 버전, 빌드 모듈, OpenSSL 연동을 기록합니다.
2. Open Source와 상용 기능, 기본 모듈과 동적·서드파티 모듈을 구분합니다.
3. 변경할 `http`, `server`, `location`, `upstream` 컨텍스트와 상속 관계를 명시합니다.
4. 성능 표현은 워커 수·연결 수·요청 크기·TLS·업스트림 지연을 고정한 측정으로만 판정합니다.
5. 클라이언트 주소 헤더는 신뢰할 프록시 주소와 Real-IP 설정을 함께 정의합니다.
6. 변경 전 구성 사본을 보관하고 `nginx -t`, reload, 실제 요청, 로그, 재시작을 순서대로 확인합니다.

문법 검사 성공은 요청 라우팅, 인증, TLS 검증, 캐시 안전성의 완료 증거가 아닙니다. 각 예제에는 기대 요청과 응답, 실패 요청, 롤백 조건이 별도로 필요합니다.
