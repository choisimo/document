# Raspberry Pi 4·5 GPIO와 주변 장치 연결 기준

이 문서는 Raspberry Pi 4와 5의 개념 연결도를 함께 담은 레거시 학습 자료입니다. 보드 리비전, 주변 장치 모델, 커넥터 규격이 다르면 핀과 케이블이 달라질 수 있으므로 보드의 공식 핀맵과 주변 장치 데이터시트를 작업 시점의 정본으로 사용합니다.

## 안전 경계

- 배선은 전원을 분리한 상태에서 구성하고, 통전 전에 물리 핀 번호와 BCM GPIO 번호를 구분해 다시 확인합니다.
- 일반 GPIO 신호는 3.3V 논리를 기준으로 하며 5V 신호를 GPIO 입력에 직접 연결하지 않습니다.
- LED 직렬 저항은 공급 전압, LED 순방향 전압, 목표 전류로 계산합니다. 다이어그램의 “저항” 표시는 특정 저항값을 보장하지 않습니다.
- 디스플레이의 전압, 백라이트 전류, DSI 커넥터·케이블 방향을 해당 제품 문서에서 확인합니다.
- Mermaid 그림은 논리 관계를 보여 주며 실제 보드 위의 물리적 방향이나 인접 핀 배치를 대신하지 않습니다.
...
연결 완료는 전원이 들어온다는 사실만으로 판정하지 않습니다. 전원 차단 상태의 단락 검사, 예상 전압 측정, 최소 기능 테스트, 재부팅 후 장치 재인식, 과열 여부 확인을 순서대로 수행합니다. 하나라도 예상과 다르면 즉시 전원을 분리하고 핀 번호·전압·극성을 다시 확인합니다.
