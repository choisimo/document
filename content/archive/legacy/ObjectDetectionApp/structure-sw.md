...
**문서 상태**: 구현 결과가 아닌 설계 초안

이 문서의 기술 스택, 인터페이스, 데이터셋, 일정, 성능 수치는 구현 또는 운영 환경에서 확인된 사실이 아니라 설계 후보와 검증 목표입니다. 실제 값으로 사용하려면 코드·배포 설정·평가 데이터·모니터링 대시보드 중 하나로 근거를 연결해야 합니다. 2025년 일정은 당시 계획을 보존한 것이며 현재 진행 상태를 뜻하지 않습니다.

| 구분 | 이 문서에서의 의미 | 확정 조건 |
| --- | --- | --- |
| 설계 후보 | 도입을 검토하는 구성요소 또는 흐름 | 담당자, 인터페이스, 비용, 장애 대응 방식 승인 |
| 성능 목표 | 달성 여부를 아직 측정하지 않은 수치 | 데이터셋·표본 수·측정 환경·산식과 함께 재현 가능한 결과 기록 |
| 운영 지표 예시 | 대시보드에 표시할 값의 형태 | 실제 계측 쿼리와 알람 규칙 연결 |
...
**AI 안전신문고**는 시민이 사진·위치·설명을 제출하고, 객체 탐지 결과를 보조 정보로 사용해 관할 기관에 신고를 전달하는 **통합 플랫폼 구상안**입니다. AI 판정은 신고 접수나 긴급 대응 여부를 단독으로 결정하지 않으며, 사용자의 확인과 기관의 접수 응답을 최종 상태로 사용합니다.
...
- **긴급 상황**: AI 분석 완료를 기다리게 하지 않고 공식 긴급 신고 수단을 먼저 안내해야 합니다.
- **자동 분류**: 모델 신뢰도가 승인 기준보다 낮거나 위치·기관 매핑이 실패하면 `검토 필요` 상태로 전환하고 사용자가 분류를 수정할 수 있어야 합니다.
- **기관 전달**: 외부 기관 API의 성공 응답과 접수 식별자를 저장한 경우에만 `전달 완료`로 표시합니다. 타임아웃은 `전달 대기`로 남기고 중복 방지 키를 사용해 재시도합니다.
- **개인정보**: 원본 이미지, 위치, 연락처의 수집 목적·보관 기간·삭제 책임자를 승인하기 전에는 운영 데이터를 수집하지 않습니다. 아래 보관 기간과 암호화 방식은 정책 후보이며 법무·보안 검토가 필요합니다.
...
이 문서는 객체 탐지, 텍스트 분석, 위치 매핑을 신고 흐름에 결합하는 설계 후보를 정리합니다. 플랫폼의 효과는 아직 입증된 결과가 아니며, 구현·보안 검토·기관 연동 시험·사용성 평가를 통과한 범위에서만 주장할 수 있습니다.
...
다음 단계의 완료 기준은 핵심 인터페이스 확정, 개인정보 영향 검토, 모델 평가 데이터와 기준 승인, 기관 전달 실패 시 복구 절차 검증입니다. 네 조건을 충족한 뒤에만 MVP 배포 여부를 판단합니다.
