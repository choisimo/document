...
마이그레이션 전에는 원본과 대상 노드의 Proxmox 버전, 스토리지 ID, 네트워크 브리지 이름, 공유 스토리지 여부를 기록합니다. 실행 중 상태와 완료 상태를 구분하고, 작업 로그가 성공으로 끝난 뒤 대상 노드에서 VM 또는 컨테이너가 시작되고 디스크·네트워크·게스트 서비스가 정상인지 확인합니다. 이 확인 전에는 원본 데이터나 백업을 삭제하지 않습니다.
