...
`CRITICAL_PROCESS_DIED`는 관찰된 정지 코드이지 단일 원인에 대한 판정이 아니다. 원인 후보는 정지 코드, 발생 시각, 직전 변경 사항, 반복 조건, 저장장치 및 메모리 검사 결과와 함께 분리해 기록한다. 재현되지 않은 설명은 가설로 표시하고, 조치 완료는 동일 조건의 재부팅 또는 부하에서 재발 여부를 확인한 시점으로 본다.
