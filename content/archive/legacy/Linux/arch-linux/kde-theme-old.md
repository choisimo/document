...
*   **설치**:
    ...
    *   글꼴 패키지 예시: `sudo pacman -S noto-fonts noto-fonts-cjk noto-fonts-emoji ttf-dejavu ttf-liberation` [10] (Noto CJK는 한국어 표시에 유용합니다).
...
한글 입력에는 입력기 설치가 필요하다. KDE Plasma 환경에서는 `fcitx5`를 사용할 수 있다.
...
KDE Plasma는 맞춤 설정 범위가 넓고 테마, 위젯, 패널 구성이 다양하다.[1, 3, 4] 추가 테마와 위젯은 KDE 스토어 [13]나 Gnome-Look.org [28, 26] 같은 커뮤니티 사이트에서 확인할 수 있다.

설정 변경은 작은 단위로 적용하고, 문제가 생기면 직전 설정으로 되돌릴 수 있도록 변경 내용을 기록한다.

## 기록의 성격

이 문서는 이전 KDE 테마 구성 절차를 보존한 역사 기록이다. 패키지 이름, 설정 경로, 화면 구성은 현재 버전과 일치한다고 가정하지 않는다. 현재 환경에 적용할 때는 최신 문서를 우선하고, 이 기록은 변경 배경과 과거 의존성을 확인하는 자료로 사용한다.
