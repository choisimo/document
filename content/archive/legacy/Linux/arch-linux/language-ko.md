...
> **진단 범위:** 이 문서는 한글 표시와 입력 문제를 로캘, 글꼴, 입력기 세 영역으로 나누어 확인합니다. 각 단계가 끝날 때 출력이나 동작을 확인한 뒤 다음 단계로 진행하십시오.

Arch Linux에서 한글이 깨져 보이거나 한영 전환이 되지 않는다면 로캘(Locale), 글꼴(Font), 입력기(Input Method Editor)를 차례로 점검합니다. 표시 문제와 입력 문제의 원인이 다를 수 있으므로 한 번에 여러 설정을 바꾸지 않습니다.
...
한글 글리프가 없는 글꼴은 문자 대신 사각형을 표시할 수 있습니다. `noto-fonts-cjk`는 한중일 글리프를 제공하는 선택지입니다.
...
설정을 마친 뒤 로그아웃하고 다시 로그인합니다. 재로그인 후 `locale` 출력, 한글 표시, 설정한 전환 키를 각각 확인하면 적용 여부를 구분할 수 있습니다.

만약 특정 프로그램(예: 웹 브라우저, 터미널)에서만 문제가 발생한다면 해당 프로그램의 글꼴 설정을 확인해 보시기 바랍니다.

---

## 보충 기록: 데스크톱 환경별 입력기 선택

아래 기록은 앞 절의 절차를 데스크톱 환경별로 보충합니다. GNOME·XFCE의 GTK 환경과 KDE Plasma의 Qt 환경에서 입력기 통합 방식이 다를 수 있다는 점을 기준으로 선택합니다.
