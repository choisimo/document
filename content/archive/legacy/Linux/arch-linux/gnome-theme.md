...
이 문서는 GNOME 테마 적용 절차를 다룬다. GNOME Shell 버전, 세션 유형(X11 또는 Wayland), 확장 기능 상태, 테마 설치 경로를 먼저 확인하며, 버전 의존 명칭은 현재 환경에서 검증되지 않으면 예시로 취급한다. 완료 기준은 셸과 애플리케이션 테마가 의도한 계정에 적용되고 재로그인 뒤에도 유지되는 상태다.
