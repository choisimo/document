...
설치 대상 디스크, 부팅 모드, 네트워크 연결을 식별한 뒤 각 단계를 진행한다. 실제 디스크 장치명이나 파티션 구성이 예시와 다르면 파티션 및 포맷 명령을 중단하고 대상을 다시 확인한다. 설치 완료는 부트 로더에서 새 시스템을 선택해 부팅하고, 네트워크와 저장장치가 의도한 구성으로 인식되는 상태를 기준으로 한다.
