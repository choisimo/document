...
> 열린 파일 테이블 내에 새로운 엔트리를 생성함 (현재 파일 위치, 파일 상태 플래그 새로 설정)
...
이 장의 경로와 파일 예제는 Linux 사용자 공간을 기준으로 읽습니다. 경로명, inode 메타데이터, 열린 파일 기술(open file description)은 서로 다른 상태이므로 같은 개념으로 해석하지 않습니다. 파일시스템별 동작을 단정하기 전에는 `stat`, `findmnt -T <경로>`, 해당 파일시스템 문서로 종류와 마운트 옵션을 확인합니다.
