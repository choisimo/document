...
> 파일, 프로세스, 네트워크, 텍스트 처리, 압축, 권한, 디스크 관리 명령 정리.
...
**삭제 위험**: `rm -rf`는 매우 위험한 명령어이다. 특히 `rm -rf /`는 시스템 전체를 삭제할 수 있다.
...
*   **`info`** - 프로그램의 Info 항목을 표시.
    *   GNU 프로젝트 프로그램은 man 페이지가 아니라 Info 형식을 제공하기도 합니다.
...
`<경로>`, `<사용자>`, `<장치>` 같은 표기는 실제 환경에서 확인한 식별자로 바꿉니다. 명령을 실행하기 전에 `--help` 또는 매뉴얼에서 현재 설치 버전의 옵션을 확인하고, `sudo`가 필요한 명령은 변경 대상과 되돌리는 방법을 먼저 기록합니다. 성공 여부는 메시지 유무가 아니라 종료 상태와 조회 명령의 결과로 판단합니다.
