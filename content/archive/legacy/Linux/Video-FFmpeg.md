...
FFmpeg가 설치되어 있지 않다면 사용 중인 배포판에 맞는 명령어로 설치한다.
...
FFmpeg로 파일을 쪼개는 방법은 크게 두 가지이며, 이 차이를 이해하는 것이 중요합니다.
    ...
    재인코딩 (기본 방식):
        ...
        단점: 인코딩 과정 때문에 시간이 오래 걸리고, 원본과 동일한 화질을 보장하기 어렵습니다. (옵션으로 품질 조절 가능)

    스트림 복사 (-c copy 옵션):
        동영상을 다시 인코딩하지 않고, 기존 데이터(비디오/오디오 스트림)를 그대로 복사해서 새로운 파일에 담기만 합니다.
        장점: 재인코딩이 없으므로 화질 손실이 전혀 없으며, 작업 속도가 매우 빠릅니다.
        단점: 동영상의 키프레임(Keyframe) 단위로만 자를 수 있습니다. 자르는 시작 지점이 키프레임이 아니면 해당 지점부터 재생이 깨지거나 음성만 나오는 문제가 발생할 수 있습니다. (키프레임: 압축된 비디오에서 기준이 되는 프레임)

## 3. 특정 시간 기준으로 파일 1개 잘라내기
...
ffmpeg -i input.mp4 -ss 00:00:10 -t 00:00:30 -c copy output.mp4
    ...
    -to 00:00:40: -t가 아니라 -to를 사용하면 종료 시간을 직접 지정할 수 있습니다. (예: -to 00:00:40는 40초 지점에서 종료)
...
ffmpeg -i input.mp4 -c copy -map 0 -segment_time 120 -f segment -reset_timestamps 1 output_%03d.mp4
    ...
    -reset_timestamps 1: 각 조각 파일이 0초부터 시작하도록 타임스탬프를 초기화합니다. 이 옵션이 없으면 재생 문제가 생길 수 있습니다.
