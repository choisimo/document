...
이 문서는 여러 배포판과 도구의 명령을 모은 기록이므로 위에서 아래로 연속 실행하는 설치 스크립트가 아닙니다. `\${username}`, `\${drive}`, `\${mount_dir}` 같은 값은 현재 시스템에서 확인한 식별자로 바꾸고, Ubuntu·Debian·Proxmox 중 적용 대상이 맞는 절만 선택합니다. 디스크 포맷, 마운트, 사용자 권한 변경은 실행 전 현재 상태와 복구 절차를 기록합니다.
