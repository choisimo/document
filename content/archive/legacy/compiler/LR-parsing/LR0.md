## LR(0) 파싱과 아이템 기반 유한 오토마타 분석

제공된 이미지 자료는 컴파일러의 구문 분석 단계에서 사용되는 **LR(0) 파싱**의 핵심 개념과, 파싱 테이블을 생성하기 위한 **유한 오토마타(Finite Automata)** 구축 과정을 설명한다.
...
**(참고: 아래 설명의 이미지 경로는 예시이다. 실제 이미지 파일명과 경로에 맞게 `images/lr0_dfa_step_1.png`처럼 문서 기준 상대 경로로 지정할 수 있다.)**
