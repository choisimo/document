## LR(0) 파싱과 아이템 기반 유한 오토마타

이 문서는 **LR(0) 아이템**, `CLOSURE`, `GOTO`, 정규 LR(0) 아이템 집합 DFA, ACTION/GOTO 테이블의 관계를 설명합니다. 예제 문법의 DFA를 완성한 뒤 ACTION 충돌을 검사해야 하며, DFA가 만들어졌다는 사실만으로 문법이 LR(0)임을 뜻하지 않습니다.
...
DFA의 한 상태는 하나의 LR(0) 아이템이 아니라, 커널 아이템에서 `CLOSURE` 규칙으로 추가되는 관련 아이템의 집합입니다. 이 관계는 상태 동치 판정이 아니라 비단말 전개 가능성을 같은 상태에 포함하는 연산입니다.

* **클로저(Closure) 규칙**:
    1.  어떤 상태에 `A → α . B β` 라는 아이템이 있고, `B`가 비단말 심볼(Non-terminal)이라면, `B`로 시작하는 모든 프로덕션 규칙의 초기 아이템(예: `B → .γ`)들도 해당 상태에 포함된다.
...
* **완료 아이템 상태**: `A → β .`가 있으면 LR(0) ACTION의 모든 단말 열에 해당 환원을 배치하려고 합니다. 단, `S' → S .`는 입력 끝 표식에서 Accept를 배치하며 일반 Reduce와 구분합니다. 같은 칸에 Shift나 다른 Reduce가 이미 있으면 충돌입니다.
...
* **최종 상태 0**: `I0 = { S' → .S, S → .(S)S, S → . }` (이미지 노트: "Initial State")
    * 이 상태는 **Shift/Reduce Conflict**를 가집니다. LR(0)에서는 완료 아이템 `S → .`가 모든 단말에 Reduce를 요구하는 반면, `S → .(S)S`는 입력 `(`에서 Shift를 요구합니다. 따라서 ACTION의 `(` 칸이 충돌하며 이 문법은 LR(0)가 아닙니다. SLR에서는 FOLLOW 집합으로 Reduce 열을 제한해 이 충돌을 피할 수 있는지 별도로 검사합니다.
...
**참고:** 아래 이미지 링크는 레거시 상대 경로입니다. 이미지가 없는 환경에서는 앞의 상태 집합과 GOTO 표를 기준 근거로 사용하며, 링크 복구 여부는 별도 문서 자산 작업으로 관리합니다.
...
*   **다음 단계로의 변화**: 이 `S' → .S` 아이템으로부터 클로저(Closure) 연산을 통해 초기 상태 I0를 구성한다.
...
정규 아이템 집합 구축은 모든 상태에서 모든 문법 기호에 대한 `GOTO`가 기존 상태와 일치하거나 새 상태로 추가되고, 더 이상 새 집합이 생기지 않을 때 완료됩니다. 그 뒤 ACTION/GOTO의 각 칸을 채워 중복 액션이 없는 경우에만 문법을 LR(0)로 판정합니다.


### 4. LR(0) 파싱의 특징, 유의점 및 핵심 요약

* **증강 문법의 중요성**: `S' → S`와 같은 증강 규칙은 파서에게 "언제 파싱을 성공적으로 종료해야 하는지"를 명확히 알려주는 역할을 합니다. 최종적으로 `S' → S.` 아이템에 도달했을 때 파싱은 성공적으로 완료됩니다.
* **상태는 '집합'이다**: DFA의 각 상태는 단일 아이템이 아닌, 문법적으로 의미가 연결된 **아이템들의 집합(클로저)**이다.
