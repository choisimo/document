이 문서는 **촘스키 계층(Chomsky Hierarchy)**과 문맥 자유 문법의 핵심 개념을 정리합니다. 계층 분류, 생성 규칙, 대응 오토마타를 구분해 설명하며, 예시는 정의의 적용 범위를 보여주기 위한 것입니다.
...
* **인식하는 언어:** **정규 언어 (Regular Languages)**
    * 예: 0개 이상의 `a` 뒤에 `b` 하나가 오는 문자열 집합 `a*b`
...
* **문법 규칙:** 일반적으로 문자열 길이를 줄이지 않는 규칙 `αAβ → αγβ`를 사용합니다. (`|γ| ≥ 1`)
    * `α`, `β`는 문맥(context)
...
|:-----:|:----------------------:|:---------------:|:----------------------:|:-----------------------------------:|
| Type-3| 정규 문법 (Regular)    | A → a \| aB      | 정규 언어 (Regular)     | 유한 오토마타 (FA)                   |
...
1. **포함 관계:** 언어 클래스는 `Type-3 ⊂ Type-2 ⊂ Type-1 ⊂ Type-0`입니다. 숫자가 작은 타입일수록 더 넓은 언어 클래스에 포함됩니다.
2. **실용성:** 전형적인 컴파일러 프런트엔드에서는 Type-3을 어휘 분석에, Type-2를 구문 분석에 주로 사용합니다.
3. **결정 가능성:** Type-0 언어의 인식기는 입력을 받아들이면 정지하지만, 받아들이지 않는 입력에서는 정지하지 않을 수 있으므로 일반적인 멤버십 판정이 결정 가능하다고 보장할 수 없습니다.
...
* **최좌단 유도 (Leftmost Derivation):** 매 단계에서 가장 왼쪽에 있는 비단말 기호를 먼저 치환합니다.
...
| `(id + id) * id`로 묶는 파스 트리 | `id + (id * id)`로 묶는 파스 트리 |
| :---: | :---: |
| `(id + id) * id` | `id + (id * id)` |
...
4. **핵심:** 모호하지 않은 문법은 각 문자열의 파스 트리를 하나로 제한합니다. 프로그램의 의미까지 하나로 보장하려면 별도의 의미 규칙과 타입 규칙이 필요합니다.
