...
_Intelligent AI-powered prompt engineering for software documentation transformation_

---

## 적용 범위와 검증 계약

- **장르:** 이 문서는 AI가 project 문서 초안을 생성하도록 하는 prompt specification입니다. 생성물이 실제 code, config, runtime, owner 결정과 일치한다는 보장은 별도 검증 없이는 성립하지 않습니다.
- **입력 전제:** authoritative source의 우선순위, repository revision, 대상 독자, 문서 범위, 제외 항목, 용어, 보안 등급과 허용된 도구를 prompt 실행마다 명시합니다.
- **사실과 추론:** source에서 확인된 사실, 계산, 사용자 제공 요구, AI 추론과 미확인 항목을 표시합니다. 근거가 없으면 채우지 말고 질문·`unknown`·검증 task로 남깁니다.
- **수치와 효과:** 작성 시간·정확도·완성도·동기화율은 baseline, 표본, 평가 rubric과 측정 결과가 없는 한 목표 가설 또는 예시입니다.
- **실패·완료:** hallucinated API, 누락된 scope, stale version, secret·PII 노출, 모순과 실행 불가능한 명령을 실패로 둡니다. requirement-to-section coverage, citation/source check, code-owner review와 실행 가능한 검증을 통과해야 완료입니다.
...
> **접근 가설**: 구조화된 prompt는 project 문서 초안의 coverage와 형식 일관성을 개선할 수 있습니다. 자동 생성 결과의 사실성·완전성은 source 기반 검증과 owner review로 판정합니다.

**📈 기대 효과**:

- ⚡ **생산성 가설**: 동일 scope의 baseline과 review·수정 시간을 포함해 작성 시간 변화를 측정
- 🎯 **품질 일관성**: 표준화된 문서 구조 및 형식
- 🔄 **지속 가능성**: 실시간 업데이트 가능한 "살아있는 문서"
- 👥 **협업 가설**: 같은 onboarding task와 cohort에서 성공률·소요 시간·질문 수를 비교
...
| ------------------ | --------------------------------- | ------------------------- | -------------- |
| **⏰ 시간 효율성** | 수동 작성으로 인한 시간 비용 | 자동화된 초안 생성 | baseline 대비 총 작성·검토 시간으로 측정 |
| **🎯 일관성** | 작성자별 style 편차 | 표준화된 template 적용 | 사전에 정의한 rubric의 일치율로 측정 |
| **🔄 최신성** | 변경사항 반영 지연 | source 변화 감지와 update workflow | revision lag와 stale claim 수로 측정 |
...
본 보고서의 핵심 목표는 AI 에이전트가 소프트웨어 프로젝트의 **"코딩 프로젝트의 전체적인 맥락이나 프로젝트 구조, 흐름도, 모듈간의 연동, 서비스 명세, 세션, 등 전반적인 내용에 대해 자세히 문서화"**할 수 있도록 하는 상세하고 견고하며 반복적으로 검증 가능한 **마스터 프롬프트 명세**를 정리하는 것이다.
...
AI 문서화 에이전트가 생성하는 결과물의 품질을 보증하려면 몇 가지 핵심 원칙을 준수한다. 이러한 원칙은 소프트웨어 문서화의 모범 사례, 구조화된 아키텍처 표현 방식, 사용자 중심의 정보 구성 방법을 포함한다.
...
AI가 생성하는 문서는 고품질 소프트웨어 문서가 갖춰야 할 핵심 원칙들을 구현한다.
...
> **💡 설계 메모**: AI 프롬프트 설계 시 각 모범 사례를 구체적인 지시사항으로 변환해 포함한다.
...
- **목적**: 초보자의 첫 경험을 안내
- **특징**: 단계별 실행, 격리된 환경, 명시적 실패 조건과 rollback
...
- **목적**: 빠른 정보 조회 지원
- **특징**: 요구 범위 coverage, source로 검증된 정확성, 체계적 구성
...
> **🤖 AI 역할 정의**:
> "전문 선임 소프트웨어 아키텍트이자 기술 작가 AI 역할. 제공된 정보와 명시된 작업을 바탕으로 소프트웨어 프로젝트를 위한 **명확하고, 포괄적이며, 정확하고, 개발자 친화적인** 문서를 생성한다. 일관된 용어 사용, 논리적 구조, 그리고 주 대상 독자(주로 소프트웨어 개발자 및 아키텍트)를 위한 적절한 수준의 세부 정보를 포함하여 소프트웨어 문서화 모범 사례를 준수한다."
...
> **🎯 문서 생성 목표**:
> "주요 목표는 **마크다운 형식**으로 전체 문서 세트를 생성하는 것이다. 각 주요 섹션은 명확하게 구분한다. 다이어그램이 필요한 경우, 특정 태그로 묶인 다이어그램의 요소, 관계 및 목적에 대한 **상세한 텍스트 설명**을 제공한다."
...
> **🔍 정확성 검증 원칙**:
> "특정 문서 섹션에 필요한 중요 정보가 제공된 입력에서 누락되었거나 모호한 경우, **명시적인 자리 표시자**를 사용하여 어떤 정보가 필요한지 밝힌다. **중요한 세부 정보를 임의로 만들지 않는다**."
...
**📋 상세 지시사항**:
제공된 아키텍처 정보를 바탕으로 각 주요 '컨테이너'를 식별하고 설명한다:
...
**📋 상세 지시사항**:
작업 2에서 식별된 각 중요 컨테이너에 대해 주요 내부 '컴포넌트' 또는 모듈을 설명한다:
...
마스터 프롬프트의 효과를 높이기 위해 다음 **최신 프롬프팅 기법들**을 통합했다. 이러한 기법들은 AI가 지시사항을 더 잘 이해하고, 일관성 있으며, 사용자가 원하는 형식과 내용에 부합하는 결과물을 생성하도록 돕는다.
...
```markdown
지침: API 엔드포인트 설명(작업 5.3) 생성 시, 다음 형식을 따른다:
...
> **🔄 지속적 품질 향상**: AI가 스스로 출력을 검토하고 개선할 수 있도록 유도한다.

**📋 자가 수정 지침**:

```markdown
지침: 각 섹션 생성 후, 다음 품질 체크리스트에 따라 자체 검토한다:
...
| **🎯 정확성** | 기술적 사실 검증 | source-backed claim 비율과 오류 severity | 불일치 수정 및 owner review |
...
> **🎯 전략적 접근**: 전체 마스터 프롬프트를 한 번에 테스트하기보다 각 작업 단위별로 나누어 테스트하여 문제점을 국소화한다.
...
```markdown
수정 전: "API 문서 생성"
수정 후: "각 API 엔드포인트에 대해 다음 정보를 포함한 완전한 문서 생성:
...
본 보고서는 AI 에이전트를 활용하여 **포괄적이고 고품질의** 소프트웨어 프로젝트 문서를 생성하기 위한 마스터 프롬프트 명세를 제시한다. 이 명세는 **C4 모델**, **Diátaxis 프레임워크**와 같은 검증된 문서화 방법론과 **연쇄적 사고**, **소수샷 예제** 등의 고급 프롬프팅 기법을 통합하여 설계했다.
...
제시된 마스터 프롬프트는 AI 에이전트가 프로젝트의 **전체적인 맥락부터 시작하여** 아키텍처의 각 계층, 주요 워크플로우, 서비스 명세, 세션 관리 등 **핵심적인 내용을 체계적으로** 문서화하도록 안내한다.
...
> **💡 핵심 변화**: 이 마스터 프롬프트는 단순한 문서 생성을 넘어 **문서화 프로세스 자체의 개선**을 목표로 한다.
...
> **💡 비전**: AI를 활용하여 생성된 문서는 **일회성 결과물이 아니라**, 프로젝트의 변화와 함께 **지속적으로 진화하는 자산**으로 관리된다.
...
> **🎯 방향성**: 본 보고서에서 제시된 전략과 마스터 프롬프트 명세는 이러한 변화와 **소프트웨어 개발 생태계 전반의 생산성 및 품질 향상**을 목표로 한다.
