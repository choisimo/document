...
# [근거 및 상태 계약]
* 입력에 저장소 경로와 기준 commit/tag, 실행 환경, 확인 가능한 설정·스키마·API 명세를 포함한다.
* 코드·설정·테스트에서 직접 확인한 내용은 `확인됨`, 여러 근거로 추론한 내용은 `추론`, 근거가 없는 내용은 `미확인`으로 표시한다.
* 계획 문서와 실제 구현을 섞지 않는다. 파일·클래스·endpoint가 없으면 예시를 실제 식별자처럼 만들지 말고 누락으로 기록한다.
* 비밀번호, token, 개인 정보는 값이 아니라 secret 이름과 주입 경로만 문서화한다.

# [PROJECT CONTEXT] - 먼저 채울 영역
...
* 해당하지 않거나 근거가 없는 섹션을 억지로 채우지 말고 `해당 없음` 또는 `미확인`과 확인 방법을 적는다.
* 모든 주요 설명에 근거 파일 경로와 symbol 또는 설정 키를 연결하고, 생성한 예시는 `예시`로 명시한다.
...
# [완료 기준]
* entrypoint부터 외부 호출과 저장까지 핵심 흐름을 실제 식별자로 추적했다.
* 인증·인가, transaction, retry, idempotency, 실패·부분 성공 상태를 근거가 있는 범위에서 설명했다.
* 문서에 기록한 build/test 명령의 실행 여부와 결과를 분리해 표시했다. 실행하지 않았다면 성공을 주장하지 않는다.
