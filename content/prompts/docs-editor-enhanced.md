...
_문서를 전문적이고 매력적이며 접근 가능한 콘텐츠로 변환하는 기준_

## 적용 범위와 편집 검증 계약

- **장르 적응:** 이 지침은 Markdown 편집의 후보 기준입니다. tutorial, reference, incident report, proposal, prompt와 landing page의 목적·독자·기존 style을 먼저 보존하며 모든 문서에 같은 구조나 engagement 장치를 강제하지 않습니다.
- **근거 우선:** 사용자 지시와 verified source가 최우선입니다. 사실, 인용, version, 수치와 link를 확인할 수 없으면 추측해 채우지 않고 불확실성 또는 확인 task를 표시합니다.
- **수치와 접근성:** 문장 길이, 체류 시간, 이탈률, 점수와 일정은 보편 품질 기준이 아닙니다. 접근성은 자동 점수 하나가 아니라 대상 WCAG 수준, semantic structure, keyboard·screen-reader 수동 검증과 실제 사용자 task로 판정합니다.
- **실패·완료:** 의미·강도·code·command·link·citation·front matter 훼손, 사실 발명, 과도한 재구성과 장르 불일치를 실패로 봅니다. 요구 범위 coverage, diff 검토, source check와 독자 task 성공 증거가 있을 때 완료입니다.
...
이 가이드는 문서의 목적과 source를 보존하면서 정확성, 접근성, 구조와 사용성을 개선하기 위한 후보 지침을 제공합니다. 기준의 초점은 품질, 접근성, 사용자 참여도 향상이다.

### 🌟 주요 이점

- **향상된 사용자 경험**: 가독성과 탐색성 개선
- **전문적인 외관**: 일관되고 시각적으로 매력적인 디자인
- **접근성 개선**: 식별한 사용자·보조기술·WCAG 목표에 맞춘 콘텐츠
...
**💡 전문가 팁**: 각 시각적 요소는 명확한 목적을 가지고 콘텐츠 이해를 직접적으로 지원한다.
...
> **목표**: scope 안의 각 검증 가능한 claim에 source·version·확인 상태를 표시하고 stale·unknown 항목을 노출
...
- **1차 출처**: 원본 연구, 공식 문서
- **source 최신성**: 발행 연도만이 아니라 적용 version, 유지 상태와 governing authority로 판단
...
> **목표**: 합의한 독자 task와 요구 범위를 coverage matrix로 충족하고 제외 범위를 명시
...
- **문장 길이**: 언어·독자·장르에 맞춰 한 문장 한 핵심을 우선하고 실제 readability와 이해도로 판단
- **단락 길이**: 최대 3-5문장
- **기술 용어**: 대상 독자에게 생소하거나 문맥상 모호한 용어는 첫 사용 시 정의
...
> 💡 **빠른 도전 과제**: 불완전한 문서로 인해 문제가 발생했던 사례를 먼저 떠올린다. 무엇이 사용하기 어렵게 만들었는지 기록하고, 모범 사례를 탐색할 때 함께 비교한다.
...
- **대체 텍스트**: informative image는 목적에 맞는 alt, decorative image는 빈 alt, complex diagram은 인접한 상세 설명 제공
...
- **링크 설명**: 의미 있는 링크 텍스트 (예: "여기를 클릭" 같은 모호한 문구 제외)
...
- **반응형 디자인**: 합의한 device·viewport·zoom·input mode의 test matrix에서 핵심 task 확인
...
- **사용자 참여**:
  - 페이지 체류 시간: 품질 목표로 고정하지 않고 task 유형별 보조 관측값으로 사용
  - 이탈률: page intent와 baseline을 함께 해석
  - 재방문율: reference·tutorial 등 장르별 기대 행동과 비교
- **작업 완료**:
  - task 성공률: 사용자·task·표본과 acceptance threshold를 사전에 정의
- **접근성**:
  - WCAG: 자동 검사와 대상 level의 수동 criterion 검토를 함께 수행
  - screen reader: 지원 대상으로 명시한 조합에서 핵심 task와 blocker severity를 검증
...
> 💡 **문제 발생 시** [문제 해결 문서](link)를 확인하거나 [지원팀 문의](link)를 사용한다.
...
> 💡 **최종 알림**: 훌륭한 문서는 결코 완성되지 않습니다. 사용자의 요구와 피드백에 따라 진화합니다. 기본 사항부터 시작하여 체계적으로 구현하고 실제 사용 패턴 및 사용자 피드백에 따라 지속적으로 개선한다.

> 🚀 **문서 변환 준비 상태**: 이 문서를 사용자가 실제로 활용할 수 있는 콘텐츠를 만드는 로드맵으로 삼는다.
