...
⚠️ **주의사항**: rm -rf는 확인 프롬프트와 휴지통 없이 대상을 재귀 삭제합니다. --preserve-root는 임의의 잘못된 경로나 빈 변수까지 보호하지 않습니다. pwd와 대상 목록을 확인하고, 변수는 따옴표로 감싸며, -- 뒤에 명시한 검토된 경로만 삭제하고 복구 가능한 백업을 먼저 준비합니다.
...
*   **killall** - Linux에서는 이름이 같은 여러 프로세스에 시그널을 보낼 수 있어 범위가 넓습니다. 다른 Unix 계열에서는 의미가 다를 수 있으므로 pgrep로 대상을 확인하고 PID 단위 kill을 우선합니다.
*   **`jobs`** - 활성 작업을 나열.
*   **`bg`** - 작업을 백그라운드로 보냄.
*   **`fg`** - 작업을 포그라운드로 보냄.
