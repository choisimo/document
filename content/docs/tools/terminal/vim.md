...
Vim은 모드 기반 텍스트 편집기다. 일반 편집기처럼 계속 입력하는 방식이 아니라, 이동과 편집 명령을 조합해 텍스트를 바꾼다. 핵심은 키를 많이 외우는 것이 아니라 `operator + motion` 또는 `operator + text object` 문법을 익히는 것이다.
...
| 조합 | 의미 |
| --- | --- |
| `dw` | 현재 위치부터 다음 단어 시작까지 삭제; 공백·줄 경계에 따른 motion 규칙 확인 |
| `ciw` | 현재 단어 변경 |
| `di"` | 따옴표 안 내용 삭제 |
| `ci(` | 괄호 안 내용 변경 |
| `d$` | 줄 끝까지 삭제 |
