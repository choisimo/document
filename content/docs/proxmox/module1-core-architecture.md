...
아래 흐름은 정상 경로의 개념도다. 모든 CLI 명령이 pveproxy/pvedaemon을 거치거나 비동기 task를 만든다는 뜻은 아니며, 실제 handler와 동기·비동기 실행 방식은 명령별로 확인한다.
...
```text
qm set 100 --cores 4
  -> command/API handler validates and updates the configuration
