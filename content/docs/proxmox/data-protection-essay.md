...
실제 지원·정합성·생성 시간은 Proxmox VE 버전, VM/CT, storage backend, guest agent와 workload에 따라 다르다. 스냅샷은 같은 delta 구현이나 즉시 생성·복원을 보장하지 않으며, rollback은 이후 데이터를 되돌리므로 의존 서비스와 별도 백업을 먼저 확인한다. 백업은 포함·제외 항목, 암호화 키, retention, application consistency와 외부 의존성을 확인한다. NAS·PBS도 같은 전원·사이트·관리 계정을 공유하면 함께 실패할 수 있어 별도 신뢰 경계의 immutable/offline 사본과 복원 시험이 필요하다. 템플릿은 변경·삭제될 수 있고 clone 수는 자원·라이선스 한도를 따른다. 보호 완료는 새 호스트에서 실제 복원과 네트워크·비밀·외부 데이터 복구가 정한 RPO/RTO를 충족하는 것이다.
