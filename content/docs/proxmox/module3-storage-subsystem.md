...
- 공유 스토리지와 로컬 디스크 전송을 포함한 migration의 지원 조건·시간·영향을 구분하지 않는다.
...
- 디스크 분리와 실제 volume 삭제는 다르다. 설치 버전의 qm disk/unlink 도움말로 대상과 삭제 옵션을 확인하고, volume을 삭제하면 되돌릴 수 없는 데이터 손실이 생길 수 있다.
