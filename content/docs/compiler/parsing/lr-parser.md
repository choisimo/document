...
LR(0) 아이템의 closure와 GOTO로 정규 항목 집합을 구성하고, 이 오토마타에서 ACTION/GOTO 표를 만드는 과정을 설명합니다.
...
3.  **완료 아이템 (Complete Item)**: 점이 RHS의 가장 오른쪽에 있는 경우입니다.
    * **예시**: `S → XYZ.`
    * **의미**: 프로덕션 RHS의 인식이 끝난 환원 후보 아이템입니다. LR(0) 표에서 실제 환원을 넣기 전에 증강 시작 규칙인지와 같은 상태의 shift/reduce·reduce/reduce 충돌을 확인합니다.
...
단계 2: I0 완성 및 GOTO(I0,S) (원본 이미지 미포함)
