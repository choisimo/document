...
- 입력은 유한한 상태·알파벳·완전한 전이 함수·시작 상태·수용 상태 집합을 가진 DFA입니다. 빠진 전이는 필요하면 명시적 sink 상태로 완성합니다.
- 먼저 시작 상태에서 도달할 수 없는 상태를 제거한 뒤, 수용 여부로 구분되는 상태 쌍을 반복적으로 분할합니다.
- 최소 DFA는 상태 이름을 제외한 동형까지 유일합니다. 최소 상태 수가 실행 시간이나 메모리 배치까지 항상 최적이라는 뜻은 아닙니다.
- 완료 증거는 분할이 더 이상 변하지 않고, 원본과 결과가 대표 수용·거부 문자열 및 동치 검사에서 같은 언어를 인식하는 것입니다.
