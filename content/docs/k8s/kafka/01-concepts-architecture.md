...
Producer는 key와 value를 serialize해 topic에 전송한다. 같은 key를 같은 partition에 배치하려면 key 직렬화, partitioner와 partition 수가 유지되어야 한다. Partition 수나 라우팅 정책을 바꾸면 이 대응 관계가 달라질 수 있다.
