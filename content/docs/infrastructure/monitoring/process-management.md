...
POSIX 계열 셸과 `systemd`를 사용하는 Linux에서 장기 실행 명령을 세션과 분리하는 방법입니다. 대화형 작업은 `tmux`/`screen`, 일회성 배치는 `nohup`, 지속 서비스는 `systemd`를 기준으로 합니다.

!!! info "적용 범위와 전제"
    셸의 job-control 및 `huponexit`, SSH 서버, `systemd-logind` 정책에 따라 세션 종료 시 신호와 프로세스 생존 여부가 달라집니다. 컨테이너나 배치 스케줄러에서는 해당 런타임의 수명주기 기능을 우선 사용하세요.

## 개요

터미널 연결이 끊기면 셸이나 터미널이 작업에 `SIGHUP`을 보낼 수 있지만, 실제 동작은 셸 옵션과 프로세스의 신호 처리 방식에 좌우됩니다. 장기 실행 작업은 표준 입출력, 재시작 정책, 종료 상태를 세션 밖에서 관리해야 합니다.
...
재시작, 부팅 연동, 권한, 로그가 필요한 지속 서비스는 `systemd` 단위로 관리하는 편이 관측과 복구에 유리합니다:
...
`echo $!`, `disown`, `systemctl start`의 성공은 작업 완료 증거가 아닙니다. 일회성 작업은 종료 코드와 예상 산출물의 크기·체크섬을 확인하고, 서비스는 `systemctl show myapp -p Result -p ExecMainStatus`와 journal을 확인합니다. 자동 재시도는 영구 오류를 반복할 수 있으므로 시작 제한, 알림, 부분 산출물 정리 또는 멱등성을 함께 정의합니다.
