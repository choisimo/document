...
양쪽 rsync·OpenSSH 버전, 파일 시스템, 사용자 권한과 ACL/xattr·링크 보존 요구를 확인한다. `-a`만으로 ACL·xattr·hardlink가 모두 보존되지는 않는다. dry-run 이후 원본이나 대상이 바뀌면 실제 작업 결과도 달라지므로 변경 중인 데이터에는 일관된 백업 시점이 필요하다.
