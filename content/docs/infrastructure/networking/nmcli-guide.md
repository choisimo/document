...
속성 지원과 `device reapply`로 반영 가능한 변경은 설치된 NetworkManager 버전에 따라 다르다. 현재 프로필을 백업하고 주소 충돌을 검사하며, 원격 변경 전 콘솔 또는 별도 관리망과 자동 롤백 수단을 확보한다. 예시 프로필 이름·장치·주소는 실제 출력으로 바꾼다.
