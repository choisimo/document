...
- QDevice 설정 후 `pvecm status`, 클라이언트 서비스 상태와 journal로 확인한다.
...
- expected votes 강제 변경은 일반 설정 절차가 아니다. 다른 partition의 writer가 배제됐다는 증거와 해당 버전의 복구 절차 없이 수행하지 않는다.
