...
util-linux, LVM2, mdadm, device-mapper로 스토리지 계층을 해체하거나 확장하는 예시입니다. 커널·도구·파일 시스템·Proxmox 버전에 따라 동작이 달라집니다.

!!! danger "파괴 작업 전 필수 조건"
    모든 장치명과 용량은 예시입니다. `lsblk`, `findmnt`, `pvs/vgs/lvs`, `mdadm --detail`로 실제 계층을 대조하고 검증된 오프호스트 백업과 복구 콘솔을 확보하세요. `wipefs`, `pvremove`, `vgremove`, `lvremove`, `--zero-superblock`은 데이터 접근을 파괴할 수 있습니다.
...
- `--force`: 중첩 파티션 테이블 등 기본적으로 거부되는 대상에도 적용. 읽기 전용 하드웨어를 쓰기 가능하게 만드는 옵션은 아님
...
### 기타 사항
- `wipefs`는 접근에 필요한 시그니처를 지울 수 있지만 데이터 영역 전체를 지우거나 안전 삭제하지 않음
- 복구 가능성은 덮어쓰기와 메타데이터 종류에 달려 있으므로 복구 또는 보안 삭제 수단으로 간주하지 않음
...
- 현재 util-linux `fdisk`도 GPT를 지원할 수 있으므로 2TiB만으로 도구를 단정하지 않고 버전과 필요한 기능에 따라 선택
- 기본 정렬은 일반 장치에 적합하지만 RAID stripe, SAN, 특수 sector geometry는 별도 확인
...
확장은 `lsblk`, `pvs/vgs/lvs`, `findmnt`, `df -hT`가 일관되고 애플리케이션 읽기·쓰기와 재부팅 후 마운트까지 확인해야 완료입니다. 중간 단계가 실패하면 다음 파괴 명령을 실행하지 말고 장치 트리와 로그를 보존해 복구 판단을 받으세요. 축소는 이 문서 범위가 아닙니다.
