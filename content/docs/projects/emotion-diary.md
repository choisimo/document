...
Emotion Diary는 사용자가 일상의 감정을 기록하고 자기 패턴을 돌아보도록 돕는 감정 일기 애플리케이션의 진행 중인 제품 구상이다. 분석·시각화 기능의 구현 여부는 별도 검증한다.

## 문서 상태와 제품 경계

- 이 문서는 진행 중인 제품 구상과 목표 아키텍처를 함께 담습니다. 표·다이어그램·API 목록은 구현 완료 표시가 없는 한 계획입니다.
- 감정 분석 점수와 AI 피드백은 자기 기록을 돕는 참고 정보이며 의료 진단, 치료 또는 위기 대응을 대신하지 않습니다.
- 일기 원문과 추론 결과는 민감 정보로 취급하고 동의, 보존·삭제, export, 모델 제공자 전송 범위와 접근 권한을 먼저 정합니다.
- 기능 완료는 UI 존재가 아니라 권한, 데이터 격리, 실패·재시도, 삭제 전파와 대표 사용자 흐름의 검증 증거로 판정합니다.
...
- 감정 점수는 선택한 모델과 계산 방식에 따른 값이며 정확도·의학적 확률·보편적 감정 비율로 해석하지 않는다. 범위·단위·모델 version과 감정 유형 기준을 서버와 클라이언트에서 일치시킨다.
- 이미지 첨부는 소유자, 크기, MIME type, 저장 위치 정책을 가져야 한다.
- 통계 집계는 원본 일기 삭제나 수정 후 stale 상태로 남으면 안 된다.

## 8. 가장 작은 예제 (Minimal Viable Example)

MVP 검토용 API 후보는 다음과 같다. route 존재만으로 완료하지 않으며 권한·동시성·실패·삭제 전파와 실제 사용자 흐름을 검증한다.
