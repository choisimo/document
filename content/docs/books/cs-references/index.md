...
> **대상과 범위:** 한국어·영어 CS 참고 문서의 정식 인덱스로 안내하는 링크 페이지다. 문서 내용이나 최신 버전을 보증하지 않으며 자동 HTTP redirect를 구현하지 않는다.

[CS References (EN/KR) 인덱스 바로가기](../cs-reference/index.md)

**완료 증거:** 생성된 사이트에서 링크가 성공 상태로 열리고 인덱스의 한국어·영어 카테고리별 문서 링크가 올바른 대상을 열어야 한다. 실패하면 상대 경로와 생성 라우트를 수정하고 재검증한다. 현재 인덱스는 제목·요약 검색과 언어·카테고리 필터를 제공한다. JavaScript가 비활성화되면 한국어·영어 문서 링크를 모두 표시한다.
...
- 사용자가 한국어/영어 카테고리별 문서 표를 보려면 canonical index로 이동해야 함을 알린다.
