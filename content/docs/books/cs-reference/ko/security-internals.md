...
S-box는 임의적이지 않다. 각 바이트 `b`은 `b⁻¹ mod (x⁸+x⁴+x³+x+1)`(GF(2⁸)의 곱셈 역수)에 매핑된 다음 아핀 변환이 적용된다. 이 구조는 다음 성질을 만든다:
...
**최소 권한 원칙(Principle of Least Privilege)**은 리팩토링 시 항상 점검하는 사항이다. 서비스 계정, API 키, IAM 역할 모두 필요한 최소한의 권한만 부여한다. 과도한 권한은 침해 발생 시 폭발 반경(blast radius)을 키웁니다.
...
**Secure by Default 패턴**은 시스템의 기본 상태가 가장 안전한 설정이어야 한다는 원칙이다. 새로운 API 엔드포인트는 기본적으로 인증이 필요하며, 공개가 필요한 경우 명시적으로 `@Public` 어노테이션을 추가한다. CORS는 기본 차단이며, 허용 도메인을 화이트리스트로 관리합니다.
...
**문제 1-1.** 모바일 뱅킹 앱이 OAuth 2.0 Authorization Code + PKCE 흐름을 사용하여 사용자의 계좌 정보에 접근하려 한다. 앱이 Authorization Server에 인가 코드를 요청하고, 이를 Access Token으로 교환한 뒤, Resource Server에서 데이터를 가져오는 전체 흐름을 그린다. 이때 PKCE의 `code_verifier`와 `code_challenge`가 각 단계에서 어떤 역할을 하며, PKCE가 없을 경우 모바일 환경에서 어떤 공격이 가능한지 설명한다.

<details><summary>힌트 보기</summary>

모바일 앱은 클라이언트 시크릿을 안전하게 저장할 수 없으므로, 인가 코드 가로채기(Authorization Code Interception) 공격에 취약하다. PKCE는 앱이 생성한 `code_verifier`의 해시(`code_challenge`)를 인가 요청에 포함시키고, 토큰 교환 시 원본 `code_verifier`를 제출하여 인가 코드를 가로챈 공격자가 토큰을 획득하지 못하도록 한다. `S256` 변환 방식과 `plain` 방식의 보안 차이도 고려 대상이다.

</details>

**문제 1-2.** 사용자가 브라우저에서 `https://bank.com`에 접속할 때, PKI(Public Key Infrastructure) 기반 인증서 체인 검증이 수행된다. 브라우저가 서버 인증서를 받은 후 Root CA까지 체인을 검증하고, OCSP로 인증서 폐지 여부를 확인하며, 최종적으로 세션 키를 교환하는 전체 흐름을 단계별로 설명한다. 만약 중간 CA(Intermediate CA)의 인증서가 만료되었다면 어떤 단계에서 연결이 실패하는지도 함께 분석한다.
...
**문제 1-3.** TLS 1.3 핸드셰이크에서 클라이언트와 서버가 세션 키를 협상하는 과정을 TLS 1.2와 비교하여 설명한다. TLS 1.3이 핸드셰이크 왕복 횟수를 줄인 방법(1-RTT, 0-RTT)과, 0-RTT 재개(resumption)가 갖는 보안 위험(replay attack)은 무엇인지 분석한다.
...
**문제 2-1.** 마이크로서비스 아키텍처에서 인증 토큰 방식을 선택한다. JWT(JSON Web Token)는 무상태(stateless)이므로 수평 확장에 유리하지만, 한 번 발급하면 만료 전까지 서버 측에서 즉시 무효화할 수 없다. 반면 서버 세션 토큰(opaque token)은 중앙 세션 스토어가 필요하다. 다음 시나리오에서 각각 어떤 방식이 더 적합한지 근거와 함께 설명한다: (1) 금융 앱에서 즉시 로그아웃 기능이 필요한 경우, (2) 글로벌 CDN 엣지에서 API 인가를 수행하는 경우.
...
**문제 2-2.** 새로운 서비스의 TLS 인증서에 사용할 암호화 알고리즘을 선택한다. RSA-2048, RSA-4096, ECDSA P-256 중에서 선택할 때, 보안 강도(비트 수준의 안전성), 서명 생성/검증 속도, 인증서 및 핸드셰이크 크기의 관점에서 각 알고리즘의 트레이드오프를 분석한다. IoT 디바이스(제한된 컴퓨팅 자원)와 고성능 웹 서버 각각에 어떤 선택이 적합한지 판단한다.
...
**문제 2-3.** API 게이트웨이에서 인증/인가 처리를 중앙화할지, 각 마이크로서비스가 독립적으로 수행할지 결정한다. 보안 정책 일관성, 단일 장애점(SPOF), 서비스 자율성, 성능(추가 네트워크 홉) 관점에서 두 접근법의 트레이드오프를 분석하고, 하이브리드 전략(게이트웨이에서 인증, 서비스에서 세분화된 인가)의 장점을 설명한다.
...
이 코드를 Prepared Statement로 리팩토링한다. 또한, ORM(예: JPA/Hibernate)을 사용하더라도 `@Query(nativeQuery = true)` 어노테이션으로 Raw Query를 작성할 때 여전히 SQL Injection에 취약한 이유를 설명하고, 안전한 대안을 제시한다.
...
`Secure`, `HttpOnly`, `SameSite` 플래그가 모두 누락된 상태다. 각 플래그의 부재가 어떤 공격(XSS, CSRF, 중간자 공격)에 노출되는지 설명하고, 올바른 쿠키 설정으로 리팩토링한다. `SameSite=Strict`와 `SameSite=Lax`의 차이도 비교한다.
...
**문제 3-3.** 레거시 시스템에서 비밀번호가 `SHA-256(password)` 해시로 저장되어 있다. Salt도 적용되어 있지 않다. 이 시스템이 레인보우 테이블 공격에 취약한 이유를 설명하고, bcrypt 또는 Argon2로 마이그레이션하는 전략을 설계한다. 기존 사용자의 비밀번호를 모르는 상태에서 점진적으로 마이그레이션하는 방법도 포함한다.
...
**문제 4-1.** 마이크로서비스 환경에서 제로 트러스트 아키텍처를 구현한다. 기존의 네트워크 경계 기반 보안(VPN + 방화벽)이 충분하지 않은 이유를 설명하고, mTLS(상호 TLS)와 SPIFFE/SPIRE를 사용하여 서비스 간 인증을 "네트워크 위치"가 아닌 "서비스 ID"로 전환하는 구체적인 구현 방식을 설명한다. SPIFFE ID(`spiffe://trust-domain/workload-id`)가 X.509 인증서의 SAN(Subject Alternative Name)에 어떻게 인코딩되는지도 함께 설명한다.
...
**문제 4-2.** 비밀번호 저장 시스템을 설계하면서 bcrypt와 Argon2의 내부 동작을 비교한다. 단순 SHA-256 해시가 레인보우 테이블 공격에 취약한 이유, Salt의 역할, 그리고 adaptive hashing(비용 인자를 조절하여 해시 계산 시간을 의도적으로 늘리는 설계)이 GPU 기반 무차별 대입 공격에 대응하는 원리를 연결하여 설명한다. Argon2가 bcrypt보다 메모리 하드니스 측면에서 우수한 이유도 분석한다.
...
**문제 4-3.** HTTPS 통신에서 대칭키 암호화와 비대칭키 암호화가 함께 사용되는 이유를 설명한다. TLS 핸드셰이크에서 RSA/ECDHE로 키 교환을 수행한 뒤 AES-GCM으로 데이터를 암호화하는 하이브리드 방식의 설계 근거를 성능과 보안 관점에서 분석하고, Perfect Forward Secrecy(PFS)를 보장하려면 키 교환 알고리즘에 어떤 속성이 필요한지 설명한다.
