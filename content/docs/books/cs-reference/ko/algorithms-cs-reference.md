...
- 이 문서의 복잡도는 별도 언급이 없으면 RAM 모델과 점근 표기 기준이다. 입력 표현, 자료구조, 수치 정밀도에 따라 실제 비용은 달라진다.
- `최적`, `보장`, `근사비`는 해당 절에 적힌 전제와 증명이 성립할 때만 사용한다. 휴리스틱의 품질은 데이터셋과 종료 조건을 함께 기록해 측정한다.
- 구현을 선택할 때는 문제 크기만 보지 말고 그래프 밀도, 메모리 한도, 온라인 입력 여부, 허용 오차를 먼저 고정한다.
- 예시 수치는 일반 결론이 아니라 계산 과정을 설명하기 위한 사례다. 운영 판단에는 대표 입력과 최악 입력을 모두 벤치마크한다.
...
### 푸시-재레이블 알고리즘: 선택 규칙과 자료구조에 따른 상계
...
| 패턴 검색 | KMP | O(N+M) | 오(남) |
| 최대 유량 | 푸시-재레이블 | 구현별 상이하며 일반 알고리즘 상계는 O(V²E) | O(V+E) |
...
- **중복 부분 문제(Overlapping Subproblems)**: 동일한 하위 문제가 반복 계산되는가? → 메모이제이션이나 테이블 기반 DP 후보
...
핵심 통찰: 그리디와 DP는 모두 최적 부분 구조를 활용할 수 있지만 동일한 기법은 아니다. 그리디는 교환 논증, 매트로이드 성질 등으로 탐욕 선택의 안전성을 별도로 증명할 수 있을 때 후보 상태를 제거한다.
...
False positive는 실제로 방문하지 않은 URL을 방문했다고 판단하여 크롤링을 건너뛰게 만든다. 재시도 경로가 없다면 일부 페이지가 수집되지 않을 수 있다. 최적 해시 함수 수는 `k = (m/n) × ln2`이고, false positive rate `p ≈ (1-e^(-kn/m))^k`이므로 p를 줄이려면 m(비트 수)을 늘려야 한다. 1%에서 0.1%로 낮추려면 최적 설정 기준으로 항목당 비트 수를 약 1.5배 늘리고 해시 함수 수도 다시 계산한다. 삭제하지 않는 표준 블룸 필터는 삽입이 완료된 키에 false negative를 만들지 않지만, 동시 처리 경쟁, 상태 유실, 서로 다른 정규화 규칙 때문에 중복 크롤링이 생길 가능성까지 제거하지는 않는다.
...
누적 n이 커지면 매번 정렬은 O(n log n)이 누적되어 전체 O(n² log n)이 된다. 최대 힙(하위 절반)과 최소 힙(상위 절반)을 균형 있게 유지하면 삽입 시 O(log n), 중위수는 두 힙의 루트에서 O(1)로 구할 수 있다. 분산 환경에서는 각 노드의 로컬 중위수를 단순 평균하면 전역 중위수가 아니다. t-digest는 분포의 꼬리 쪽에 더 작은 클러스터를 배치하는 병합 가능한 근사 스케치다. 공간 사용량과 오차는 상수가 아니라 압축 매개변수, 입력 순서, 병합 방식에 따라 달라지므로 허용 오차를 데이터로 검증해야 한다.
