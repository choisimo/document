# Android 및 모바일 내부 동작 학습 및 기록 노트
...
Android 앱은 Kotlin 코드와 UI만으로 동작하지 않는다. Activity 호출은 Binder IPC를 지나 system_server로 전달되고, DEX는 ART에서 JIT/AOT로 최적화되며, UI는 Choreographer/Vsync/SurfaceFlinger/GPU 합성 경로를 통과한다. Compose 상태 변경도 SlotTable과 Recomposer를 거쳐 실제 화면 갱신으로 이어진다.

이 문서는 원문 한국어 Android 내부 문서를 앱 실행, IPC, 런타임, 렌더링, 메모리, 장치 스택의 data flow 중심으로 재작성한다.

## 2. 현재 나의 상태 (Baseline)

- Activity, Fragment, ViewModel, Compose, Room, Coroutine 같은 앱 개발 요소는 알고 있다.
- Binder, ART, SurfaceFlinger, HAL, zRAM, LMK 같은 시스템 내부와 앱 코드의 연결이 약하다.
...
- Android 메모리 누수와 프로세스 종료/복원 경로를 런타임 구조로 설명할 필요가 있다.

## 3. 도달하고 싶은 목표 (Target State)

- 앱 호출이 Framework, Binder, system service, HAL, kernel로 내려가는 흐름을 설명한다.
- Binder가 parcel을 kernel buffer와 mmap으로 전달하는 이유를 이해한다.
- ART의 DEX, JIT, profile-guided AOT 흐름을 구분한다.
...
flowchart TD
    A[App code] --> B{요청 종류}
    B -->|시스템 서비스| C[Binder IPC]
    B -->|코드 실행| D[ART Interpreter/JIT/AOT]
    B -->|UI 갱신| E[Choreographer/View/Compose]
    B -->|렌더링| F[RenderThread/GPU/SurfaceFlinger]
    B -->|하드웨어| G[HAL/Kernel driver]
    C --> H[system_server]
    D --> I[Native code or bytecode 실행]
    E --> F
    F --> J[Display frame]
    G --> J
```

Android의 여러 Framework·앱 서비스 호출은 Binder를 통해 system_server나 vendor 서비스로 전달된다. 소켓, 파이프, 공유 메모리와 장치별 IPC 경로도 있으므로 중요한 작업이나 교차 프로세스 통신이 모두 Binder를 통과한다고 일반화하지 않는다.
...
- Camera/BT/WiFi 요청은 permission, HAL capability, lifecycle release가 모두 맞아야 한다.
...
이 예제는 Android API 호출이 단순 함수 호출처럼 보이지만 실제로는 kernel Binder driver와 system_server의 상태 검사를 거치는 IPC임을 보여준다.
...
- [x] Binder IPC 최소 예제로 앱 호출이 system_server로 넘어가는 과정을 설명했다.
- [x] Compose recomposition, lifecycle, memory leak, ANR 실패 사례를 포함했다.
- [x] 모바일 앱 설계의 백그라운드 작업과 상태 복원 기준을 정리했다.
- [x] 원문 한국어 Android/mobile 문서를 12개 섹션 템플릿으로 재작성했다.
