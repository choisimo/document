...
> **읽기 계약:** 이 문서는 그래픽, 게임 엔진, IoT, IaC, SRE, 테스트, 데이터베이스와 운영체제의 대표 메커니즘을 묶은 학습용 개요입니다. 별도 참고문헌·제품 버전·벤치마크 환경이 없는 설명은 **개념 모델**, 제품명과 구조가 연결된 설명은 **구현 예**, 지연시간·메모리·배율은 **예시 수치**로 구분합니다. 수치를 적용하려면 엔진·GPU·프로토콜·도구 버전, 하드웨어, 구성, 부하와 측정 시점을 기록해야 합니다. 항목은 공식 사양 또는 재현 가능한 측정, 성공 조건, 실패·복구 경로가 연결된 뒤에만 검증 완료로 봅니다. 전제가 맞지 않으면 단정문을 유지하지 않고 미확정 상태로 되돌립니다.
...
**Early-Z**는 구현과 렌더 상태가 허용할 때 셰이더 실행 전에 가려진 조각을 거부해 작업을 줄입니다. 셰이더의 깊이 쓰기, `discard`, 알파 테스트와 일부 부작용은 조기 테스트를 제한하거나 늦출 수 있지만, 항상 기능이 완전히 비활성화된다고 단정할 수는 없습니다.
...
- QoS 2: 한 MQTT 전송 구간에서 중복 전달을 억제하는 4-메시지 핸드셰이크. 브로커 이후의 저장·처리까지 종단 간 정확히 한 번을 보장하지는 않습니다.
...
**지연시간 예시**: 특정 근거리 엣지 구성은 10ms 미만, 클라우드 왕복은 50~200ms가 될 수 있습니다. 이는 거리·무선 규격·혼잡·브로커·실행 주기에 따라 달라지므로, 실시간 제어 가능 여부는 측정된 최악 지연과 지터 예산으로 판정합니다.
...
멱등성을 제공하는 모듈은 현재 상태를 읽고 원하는 상태와 비교한 뒤 필요한 변경만 수행해야 합니다. 모든 모듈이 멱등적이거나 검사 모드를 지원하는 것은 아니며, `--check` 결과도 외부 부작용과 실제 적용 성공을 보장하는 전체 테스트가 아닙니다.
...
flowchart TD
    ...
    Runner -->|all pass| Pass["No counterexample found in 100 generated samples"]
```
...
**워프 발산**: 워프의 스레드가 서로 다른 분기(`if (threadId % 2)`)를 취하면 활성 경로가 직렬화되고 다른 레인은 마스킹될 수 있습니다. 두 갈래가 반씩 갈리는 예에서는 순간 활용도가 50%가 될 수 있지만, 분기 수와 경로 길이에 따라 더 낮거나 높을 수 있습니다.
...
`numactl --membind=0`은 지정 노드로 메모리 할당 정책을 제한합니다. 원격 NUMA 접근은 지연과 GC 비용을 높일 수 있지만 증가율은 토폴로지·수집기·힙·부하에 따라 측정해야 합니다. JVM 옵션의 이름과 효과도 대상 JDK와 수집기에서 확인합니다.
...
시스템 설계에서 **API 설계 패러다임 선택**은 중요한 구조 결정입니다. REST는 리소스 중심 인터페이스를 제공하지만 오버페칭·언더페칭이 생길 수 있고, GraphQL은 요청 형태를 클라이언트가 지정하는 대신 N+1 쿼리와 캐싱 정책을 별도로 다뤄야 합니다. gRPC는 스키마 기반 바이너리 전송과 스트리밍을 제공하지만, 최고 성능 여부는 메시지 크기·연결·구현·브라우저 경계에서 측정해야 합니다.
...
**캐싱 전략 선택**은 데이터 일관성과 성능 사이의 핵심 트레이드오프입니다. Cache-Aside는 애플리케이션이 캐시와 DB를 직접 관리하여 유연하지만, 캐시 무효화 로직이 복잡해질 수 있습니다. Write-Through도 캐시·저장소 사이의 원자성과 실패 처리가 설계되어야만 목표 일관성을 제공하며 쓰기 지연이 증가할 수 있습니다. Write-Behind는 쓰기 경로를 단축할 수 있지만 유실·재순서화·중복 반영에 대비해야 합니다.
...
**기술 부채 관리**는 코드 품질과 개발 속도 사이의 핵심 설계 원칙입니다. 의도적 기술 부채(빠른 출시를 위한 의식적 선택)와 비의도적 기술 부채(설계 부족 또는 이해 부족)를 구분해야 합니다.
