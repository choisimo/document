...
| Ansible | module result, changed flag | 사용하는 모듈의 멱등성·check mode 지원을 확인하고 반복 적용 및 실제 부작용을 검증한다. |
...
- Shader의 깊이 쓰기나 `discard`가 GPU와 렌더 상태에 따라 Early-Z를 제한하거나 늦춰 overdraw 비용을 늘릴 수 있다. 항상 완전히 비활성화된다고 단정하지 않는다.
