# 마이크로서비스 내부: 통신 계약, 전달 의미와 장애 격리

> 소스 합성: 서비스 메시, API 게이트웨이, 서비스 간 통신, 분산 추적, 서비스 검색 및 탄력성 패턴을 다루는 마이크로서비스 참조 서적(comp 107, 112, 147–151, 163–164, 370).

## 적용 범위와 판정 기준

- Istio, Envoy, Kubernetes, Resilience4j와 Kafka의 동작은 버전과 배포 모드에 따라 달라진다. 아래 포트·TTL·기본값은 구성 예시이며 현재 설정을 우선한다.
- 요청, 이벤트, 데이터베이스 쓰기의 성공을 분리한다. 전달 보장, 멱등성 키, 재시도, 순서, 보상 실패와 관찰 가능한 최종 상태를 계약으로 적는다.
- `분리`, `격리`, `정확히 한 번`은 구성 요소 하나의 속성으로 확정하지 않는다. 공유 스키마·브로커·인증서·DNS와 운영 팀의 결합도까지 추적한다.
- 성능 수치는 동일한 페이로드, 연결 재사용, 암호화, 샘플링과 부하 분포에서 측정하고 p50뿐 아니라 p95/p99와 실패율을 남긴다.
...
```mermaid
flowchart TD
    subgraph ControlPlane["Control Plane의 개념적 역할 (릴리스별 구성 상이)"]
...
- **Envoy 트래픽 캡처**는 사이드카 모드의 iptables `REDIRECT` 예시로 설명할 수 있다. 제외 포트, TPROXY, ambient 모드와 CNI 구성에서는 경로가 달라지므로 실제 규칙과 프록시 설정을 확인한다.
- **xDS**는 SotW와 delta 방식 등 여러 전송 형태를 지원한다. ACK/NACK는 구성 수락 상태를 나타내며 애플리케이션 트래픽의 성공이나 자동 롤백을 뜻하지 않는다.
- **Protobuf** 크기는 필드 번호, wire type, 값 분포와 JSON 표현에 따라 달라진다. 고정된 3~10배를 전제하지 말고 대표 메시지를 직렬화해 비교한다.
- **회로 차단기 반 개방**의 허용 프로브 수와 동시성은 라이브러리 설정에 따라 달라진다. 성공·실패 임곗값과 재개 상태를 함께 정의한다.
...
CQRS(Command Query Responsibility Segregation)는 읽기 모델과 쓰기 모델을 분리해 서로 다른 요구에 맞출 수 있다. 이벤트 소싱과 결합하면 보존된 이벤트 범위에서 상태 재구성과 감사 추적을 지원하지만 삭제·정정·스키마 진화·외부 효과까지 자동으로 복원하지는 않는다. 도입 비용은 모델 동기화, 운영 도구와 팀 역량으로 평가한다.

도입 판단 기준:
- **도입 검토**: 읽기와 쓰기의 모델·확장 요구가 실질적으로 다르고, 지연된 프로젝션과 별도 운영 비용을 감수할 수 있음
...
실시간 재고 확인에는 동기 호출을 후보로, 즉시 완료가 필요 없는 이메일 발송에는 비동기 큐를 후보로 비교한다. Circuit Breaker의 fallback은 캐시 신선도와 잘못된 재고 응답의 위험을 계약에 포함한다. Kafka offset 커밋만으로 외부 이메일 발송의 멱등성이 보장되지는 않는다. 주문 ID와 알림 유형을 유일 키로 둔 처리 기록, 공급자 idempotency key 또는 상태 머신으로 중복 부작용을 막는다.
...
공유 스키마의 직접 접근은 관련 서비스 사이에 변경 결합을 만들고 독립 배포 범위를 줄일 수 있다. 실제 독자와 writer를 식별한 뒤 읽기 계약 분리, 쓰기 소유권 이전, 물리 저장소 분리의 단계와 되돌리기 조건을 정한다. Outbox는 DB 변경과 이벤트 레코드 사이의 원자성을 제공하지만 CDC 지연·중복·순서 문제는 남으므로 재처리와 소비자 멱등성을 함께 설계한다.
...
DNS 캐시가 만료된 클라이언트는 CoreDNS 장애 중 새 이름 해석에 실패할 수 있다. 기존 연결이나 유효한 로컬 캐시를 쓰는 요청까지 즉시 실패한다고 일반화하지 않는다. CoreDNS 복제본·PDB·토폴로지 분산, NodeLocal DNSCache, 클라이언트 캐시와 재시도를 각각 시험하되 과도한 재시도 폭주도 확인한다. 대체 서비스 검색 도구는 기능뿐 아니라 별도 제어면의 장애와 운영 비용까지 비교한다.
...
Jaeger에서 임계 경로의 긴 span을 후보로 찾되, 부모·자식의 겹치는 시간과 self time, 큐 대기, 누락된 span을 구분해야 병목을 판정할 수 있다. 애플리케이션이 하위 호출에 컨텍스트를 전파하지 않으면 trace가 분리될 수 있다. 샘플링 변경은 제어면 설정만이 아니라 head/tail sampling의 위치, 이미 버린 trace의 복구 불가, Collector 용량과 개인정보 비용을 포함해 설계한다.
