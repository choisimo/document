...
> 소스 합성: 이 문서에서 다루는 주요 데이터 구조에 대한 메모리 레이아웃, 포인터 메커니즘, 캐시 동작, 상각 복잡성 및 알고리즘 불변성을 다루는 데이터 구조 참고서(comp 180–186, 189, 191–192, 213, 241–243).


## 범위와 검증 계약

이 문서는 추상 자료형의 성질과 대표 구현의 메모리 동작을 연결합니다. 언어 런타임이나 표준 라이브러리의 특정 구현을 보편 규칙으로 확장하지 않습니다.

- **범위:** 배열, 연결 구조, 해시 테이블, 트리, 힙, 그래프와 확률적 자료구조의 대표 연산을 다룹니다. 라이브러리별 성장률·임계값·메모리 배치는 버전이 명시될 때만 구현 사실입니다.
- **표기 전제:** 복잡도에는 연산, `n`의 의미, 최악·평균·상각 중 어느 경우인지가 함께 있어야 합니다. 해시의 기대 `O(1)`은 적절한 분포와 리사이즈를, 동적 배열의 상각 비용은 선택한 성장 정책을 전제로 합니다.
- **수치 전제:** 2배 성장, 8바이트 포인터, 64바이트 캐시 라인, Java `HashMap`의 용량·부하율·트리화 임계값과 “몇 배 빠름”은 특정 구현 또는 하드웨어의 예시입니다. 포인터 압축, 정렬, allocator, CPU와 런타임 버전에 따라 달라집니다.
- **사실·추론:** ADT의 불변식과 증명된 복잡도는 모델 안의 사실이고, 캐시 적중·메모리 사용량·실행 시간은 측정 가설입니다.
- **완료 기준:** 구현·컴파일러·런타임·CPU·입력 분포·워밍업을 고정하고 시간뿐 아니라 할당량, cache miss, branch miss를 반복 측정해야 구현 선택의 우위를 판정합니다.
