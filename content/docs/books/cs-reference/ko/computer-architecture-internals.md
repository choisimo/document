...
- 5단계 RISC 파이프라인, 단일 캐시, MESI 상태도는 개념을 격리한 **교육 모델**이다. 현대 코어의 실제 프런트엔드, 비순차 실행, 캐시 일관성 구현과 일대일로 대응하지 않는다.
- ISA가 허용하는 동작, 특정 마이크로아키텍처의 구현, 한 시스템에서 측정한 결과를 분리한다. CPU 모델, 스테핑, 마이크로코드, 주파수, 메모리 구성과 측정 도구를 기록한다.
- 캐시·TLB·ROB 크기와 지연시간은 세대별 수치다. 아래 표의 이름과 세대를 근거 없이 "최신"으로 확장하지 않는다.
- 성능 결론은 평균만으로 확정하지 않고 IPC, miss rate, stall 원인, p95/p99 지연과 반복 분산을 함께 제시한다.

---

## 1. 명령 파이프라인: 5단계 RISC 모델

파이프라이닝의 기본 통찰은 여러 명령의 단계를 겹쳐 처리하는 데 있습니다. 아래 단일 발행 5단계 모델은 hazard가 없고 모든 단계가 한 사이클일 때 정상 상태 처리량이 1명령/사이클에 접근합니다. 실제 superscalar 코어의 발행 폭과 IPC는 더 크거나, 의존성과 병목 때문에 더 작을 수 있습니다.
...
## 11. 세대를 고정한 CPU 사례(Intel Ice Lake / AMD Zen 3)
...
비순차 실행 창(ROB, 스케줄러, 물리 레지스터)은 순차 명령 스트림에서 ILP를 찾는 자원 중 하나입니다. 창을 키워도 데이터 의존성, 분기 오예측, 메모리 대기, 프런트엔드 폭이 병목이면 처리량이 비례해 증가하지 않습니다. 표의 값은 제품 변형과 출처에 따라 달라질 수 있으므로 설계 문서나 검증된 마이크로벤치마크로 확인합니다.
...
**메모리 일관성 모델**은 아키텍처와 언어 런타임 사이의 중요한 계약입니다. Sequential Consistency(SC), x86 계열의 TSO, ARM/RISC-V의 약한 순서 모델은 허용 순서와 동기화 명령이 다릅니다. 약한 모델이 모든 재정렬이나 항상 더 높은 성능을 뜻하지 않으며, 프로그래머가 직접 배리어를 넣는지 컴파일러·표준 라이브러리의 원자 연산을 쓰는지도 구분해야 합니다.

| 모델 | 허용되는 재정렬 | 하드웨어 | 프로그래밍 난이도 |
|---|---|---|---|
| SC (Sequential Consistency) | 없음 | 성능 제한 큼 | 쉬움 |
| TSO (x86) | Store→Load만 | 적절한 타협 | 중간 |
| Relaxed (ARM/RISC-V) | ISA가 정의한 범위에서 더 많은 순서 완화 | 구현 선택 폭 큼 | 동기화 의미를 명시해야 함 |
...
버스 스누핑: 코어 1이 BusRd 발행 → 관련 에이전트가 스누핑 → 코어 0이 데이터를 제공 → 코어 0: M→S, 코어 1: I→S라는 교육 모델로 설명할 수 있다. 디렉토리 방식에서는 홈 노드가 소유자와 공유자 정보를 찾아 필요한 메시지를 전달한다. 브로드캐스트 범위는 코어 수에 따라 커지지만 디렉토리도 소유자 전달, 공유자 무효화, 혼잡과 디렉토리 저장 비용이 있으므로 전체 통신을 O(1) 메시지로 단정하지 않는다.
...
단순한 GHR 계열은 보통 더 작은 상태와 낮은 접근 비용을 목표로 하고, TAGE 계열은 여러 히스토리 길이와 태그 비교로 더 복잡한 상관을 포착한다. 정확도와 전력은 테이블 예산, 파이프라인, 분기 분포에 따라 달라지므로 고정된 92%·97%나 "필수"로 결론내리지 않는다. (a)~(c)는 각 후보를 같은 저장 용량과 전력 제약에서 시뮬레이션하고 MPKI, 오예측 벌점, 에너지로 비교한다.
...
같은 캐시 라인의 서로 다른 변수를 여러 코어가 갱신한다면 거짓 공유를 의심하고 정렬 또는 패딩의 전후를 측정합니다. 같은 변수를 갱신한다면 샤딩된 카운터나 주기적 합산처럼 요구 일관성에 맞는 대안을 검토합니다. `perf c2c`는 HITM 등 캐시 간 전송의 증거를 수집하는 도구이지만, 심볼·샘플링·플랫폼 지원을 확인하고 다른 프로파일과 함께 원인을 판정해야 합니다.
...
`1536 × 4KB = 6MiB`는 1536개 엔트리를 해당 페이지 크기에 모두 쓸 수 있다고 가정한 이론상 최대 reach다. 실제 TLB는 페이지 크기와 단계별로 나뉘고 associativity와 다른 매핑의 간섭도 받는다. 같은 1536개를 가정하면 2MiB 페이지의 산술상 reach는 3GiB지만 실제 지원 엔트리 수를 CPU 자료로 다시 계산해야 한다. 대형 페이지는 TLB miss를 줄일 수 있는 대신 연속 메모리 확보, 내부 단편화, 컴팩션·마이그레이션 비용을 늘릴 수 있으므로 miss counter와 메모리 사용량으로 전후를 비교한다.
