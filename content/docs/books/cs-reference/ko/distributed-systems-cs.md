...
이 문서는 합의, 복제, 일관성과 장애 감지의 핵심 모델을 설명합니다. 보장은 알고리즘 이름만이 아니라 네트워크·시간·장애·저장소 전제와 함께 읽어야 합니다.

- **시스템 모델:** 동기·부분 동기·비동기 네트워크, crash-stop·crash-recovery·Byzantine 장애, 메시지 손실·중복·재정렬, quorum과 안정 저장소 가정을 먼저 명시합니다.
- **보장 구분:** safety는 잘못된 상태가 발생하지 않는다는 주장이고 liveness는 전제 아래 결국 진행한다는 주장입니다. timeout과 heartbeat는 관측 수단이며 비동기 구간에서 느린 노드와 실패한 노드를 확정적으로 구별하지 못합니다.
- **CAP 범위:** CAP의 C는 보통 linearizability, A는 partition 동안에도 비실패 노드가 받은 모든 요청에 응답하는 성질입니다. P는 기능 옵션이 아니라 분석할 네트워크 사건이며, CP/AP 표시는 partition 구간의 요청별 정책을 요약할 뿐 시스템 전체의 영구 분류가 아닙니다.
- **수치·확률 전제:** `O(log N)` gossip 수렴은 무작위 피어 선택과 혼합 가정 아래의 기대치입니다. Phi 값, timeout, clock uncertainty, quorum latency와 메시지 수는 분포·토폴로지·부하·구현에 종속되며 신뢰도나 상한으로 그대로 읽지 않습니다.
- **정합성 판정:** 하나의 원자적 트랜잭션이 `x=1` 다음 `y=1`을 쓰는데 다른 트랜잭션이 `y=1, x=0`을 관측했다면 이를 설명하는 직렬 순서가 없으므로 serializable하지 않고 strict serializable하지도 않습니다.
- **완료 기준:** invariant, 선행 조건, 실패 일정, 재시도·중복·재정렬 시나리오와 관측 가능한 판정식을 제시하고 deterministic simulation 또는 fault injection으로 반례가 없음을 확인해야 보장 검증을 완료로 판정합니다.
...
과제: **노드 A의 관점에서 보면 노드 B 충돌은 매우 느리게 응답하는 노드 B와 구별할 수 없습니다.** timeout은 heartbeat와 통신 관측을 판정하는 한 메커니즘이며, 비동기 구간에서는 지연과 실패를 확정적으로 구분하지 못합니다이지만 최대 메시지 지연을 알지 못하면 올바른 시간 초과를 선택하는 것은 불가능합니다.
...
- 이전 라운드에서 어떤 값이 승인된 경우 제안자는 해당 값을 제안한다.
...
**샘플링 결정 전파**: `traceparent` 플래그 바이트는 샘플링 결정을 인코딩한다. 루트 범위가 샘플링(확률적, 1%)을 결정하면 모든 다운스트림 서비스는 해당 결정을 **상속**한다. 이 방식은 부분 추적이 아닌 전체 추적을 보장한다.

---

## 11. 가십 프로토콜: 전염병 정보 유포

Gossip(Cassandra, Consul, Redis Cluster에서 사용)은 무작위 피어 선택과 충분한 혼합을 가정할 때 기대 O(log N) 라운드로 정보를 퍼뜨립니다.
...
여기서 `P_later`은(는) 과거 도착 간 시간의 가우스 모델을 고려하여 `t` 시간 이후에 다음 하트비트가 도착할 확률입니다. Φ=1의 수치는 선택한 도착 시간 모델에서 계산한 예시 실패 신뢰도. Φ=8의 수치도 같은 모델의 tail probability 예시.
...
네트워크 파티션이 복구된 후 복제본은 분산된 데이터를 조정한다. 머클 트리는 이를 효율적으로 만듭니다.
...
설계적 교훈: **이론적 완결성보다 이해하고 구현할 수 있는 시스템이 실무에서 더 많은 가치를 만든다.** Paxos의 좋은 구현체는 소수이지만, Raft의 좋은 구현체는 수십 개다.
...
**문제 2-1.** 전자상거래 플랫폼이 사용자 세션 저장소를 설계한다. CAP 정리에 의해 네트워크 파티션 시 CP(Paxos/Raft 기반)와 AP(Dynamo 스타일 최종 일관성) 중 하나를 선택한다. 다음 시나리오 각각에 적합한 선택과 그 이유를 설명하시오: (a) 장바구니 데이터로 구매 결정 직전에 '재고 수량'을 표시하는 경우, (b) '최근 본 상품' 추천 목록을 표시하는 경우, (c) 결제 시점에 재고를 차감하는 경우.
...
Google Spanner의 TrueTime은 특정 시간 인프라와 API 계약에 의존합니다. 다른 클라우드도 시간 동기화 서비스를 제공할 수 있으므로 공급자 이름만으로 가능 여부를 단정하지 말고, 노출되는 uncertainty bound, 장애 모델, holdover와 SLA를 비교해야 합니다.
...
T1이 하나의 원자적 트랜잭션에서 `x=1` 뒤 `y=1`을 기록했다면 T2의 `y=1, x=0` 관측을 만드는 직렬 순서는 없습니다. 따라서 이 실행은 serializable하지 않으며, 실시간 순서를 추가로 요구하는 strict serializable도 아닙니다.

</details>

**문제 4-3.** 가십 프로토콜, CRDT, 벌터 클럭은 모두 **분산 상태 수렴**을 다루지만 보장 범위가 서로 다르다. 100노드 클러스터에서 노드 방금 리스트(IP 차단 목록)를 모든 노드에 전파해야 할 때, 세 방식 각각의 장단점을 분석하시오. 특히 (a) 수렴 속도, (b) 일시적 불일치 동안의 보안 위험, (c) 네트워크 대역폭 사용량을 비교하시오.

<details><summary>힌트 보기</summary>

가십: 팬아웃(프로토콜·팬아웃·중복 억제·라운드 수에 따라 달라지는 메시지 비용 메시지)이지만 확률적 수렴(O(logN) 라운드). 실시간 차단이 아니라면 수 초 지연 허용 가능. CRDT(OR-Set): 노드 추가만 있으면 G-Set으로 충분. 충돌 없이 병합되지만 전체 상태 전송 시 대역폭 큼. 벡터 클럭: 인과적 순서 보장하지만 N개 노드로 클럭 크기 O(N). 보안에는 리더 기반 브로드캐스트(즉시 적용)가 더 적합할 수 있습니다.
