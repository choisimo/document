...
이 문서는 InnoDB와 SQL Server를 중심으로 페이지, 인덱스, MVCC, WAL, 실행 계획의 대표 경로를 설명합니다. 서로 다른 엔진의 내부 구조와 기본값을 하나의 보편적 관계형 데이터베이스 규칙으로 취급하지 않습니다.

- **범위:** 엔진 이름이 붙은 구조는 해당 제품 계열의 설명입니다. 페이지 크기, doublewrite, redo 파일, 격리 수준, 잠금과 온라인 DDL 동작은 제품·버전·에디션·설정에 종속됩니다.
- **내구성 전제:** “커밋”과 “내구성”은 WAL 순서만으로 판정하지 않습니다. 커밋 flush 정책, `fsync` 계층, 저장 장치의 캐시·barrier, 복제 동기화와 복구 시험까지 명시해야 합니다.
- **사실·예시·추론:** 페이지와 로그의 불변식은 지정된 엔진 모델 안의 사실이고, 비용식·선택도·적중 시간·조각화율·읽기/쓰기 비율은 설명용 예시 또는 튜닝 가설입니다.
- **수치 사용:** 16KB/8KB 페이지, 69% 채움, 2MB doublewrite, 나노초·초·분 범위와 비율 임계값은 보편 기준이 아닙니다. 실제 스키마, 행 폭, 데이터 분포, 캐시 상태, 동시성, 저장 장치와 실행 계획으로 다시 측정합니다.
- **완료 기준:** 변경 전후의 실행 계획, 실제 행 수, 논리·물리 I/O, 잠금 대기, redo/flush, 지연 분포를 수집하고 crash-recovery와 rollback 시나리오를 통과해야 성능·내구성 개선을 완료로 판정합니다.

---

## 1. 스토리지 엔진 아키텍처 — 페이지, 익스텐트 및 버퍼 풀

이 문서가 다루는 InnoDB와 SQL Server 같은 페이지 기반 관계형 엔진은 페이지 추상화로 저장소를 관리합니다. 페이지 구조는 해당 엔진의 I/O와 실행 계획을 분석하는 한 출발점이며, 모든 관계형 엔진이나 모든 병목에 동일하게 적용되지는 않습니다.

### InnoDB 페이지 레이아웃(일반적인 설정 예: 16KB)
...
**이중 쓰기 버퍼**: 더티 페이지를 테이블스페이스로 플러시하기 전에 InnoDB는 이를 버전과 설정에 따라 형태와 크기가 달라지는 이중 쓰기 영역(여기서는 2MB, 128페이지 예시)에 순차적으로 씁니다. 이는 충돌 중에 부분 페이지 쓰기(찢어진 페이지)로부터 보호합니다. 페이지 체크섬이 실패한 경우 복구 과정은 유효한 이중 쓰기 사본이 존재하고 해당 기능이 활성화된 조건에서 이를 사용할 수 있습니다.
...
리프 페이지가 용량에 도달하면(이 예시에서 업데이트 공간을 남긴 채움 상태 약 69%):
...
**LSN(Log Sequence Number)**: 리두 로그에 대한 바이트 오프셋을 단조롭게 증가시킵니다. 모든 페이지 헤더는 `FIL_PAGE_LSN` = 마지막 수정 LSN을 저장합니다. `page_LSN`과 checkpoint의 관계만으로 트랜잭션 내구성을 판정할 수는 없으며, 커밋된 redo의 flush 정책과 복구 절차를 함께 확인해야 합니다.
...
> 아래 수치는 보편적 기준선이나 SLA가 아니라 설명용 범위입니다. 엔진·버전·스키마·데이터 분포·캐시·동시성·스토리지 조건을 고정한 측정으로 대체해야 합니다.
...
이 문서가 설명하는 대표적인 바이트 수명 주기: SQL 텍스트 → AST → 비용 추정 계획 → 반복자 풀 → 버퍼 풀 페이지 → MVCC 실행 취소 체인 → 필드 바이트. 쓰기 시: 실행 취소 로그(이미지 전) → 더티 페이지(메모리 내) → 다시 실행 로그(WAL, 내구성) → .ibd로 백그라운드 플러시. WAL 순서 규칙은 이 복구 경로의 핵심 전제입니다. 실제 데이터 페이지가 디스크에 있는지 여부에 관계없이 커밋 정책이 해당 redo의 안정 저장소 flush를 요구하고 저장소 계층이 이를 준수한 시점에 데이터는 복구 가능한 것으로 간주할 수 있습니다.
...
실무에서의 결정 기준:
- **갱신이 빈번하고 무결성 비용이 큰 경우**: 정규화를 우선 검토하되 실제 쿼리와 트랜잭션 경계를 측정합니다.
- **읽기가 압도적이고 JOIN 비용이 검증된 병목인 경우**: 동기화·재처리·복구 비용을 포함해 역정규화를 검토합니다.
...
**WAL**은 데이터를 수정하기 전에 변경 사항을 로그에 먼저 기록한다. 커밋 시점에는 로그만 fsync하면 되므로 리두 로그의 순차적 flush로 커밋 비용을 줄일 수 있지만, 내구성은 flush 정책과 저장소 보장을 전제로 한다. 실제 데이터 페이지는 백그라운드에서 비동기적으로 플러시된다.
...
여러 관계형 데이터베이스 실행 엔진은 **Volcano(Iterator) 모델**을 사용한다. 각 연산자(Scan, Filter, Join, Sort)가 `open()`, `next()`, `close()` 인터페이스를 구현하는 **Iterator 패턴**이다.
...
Hash 인덱스는 주로 **등치 검색(=)**에 적합하며 정렬 순서가 필요한 일반적인 범위 검색(`BETWEEN`, `<`, `>`)에는 부적합하다. B+Tree는 정렬된 구조이므로 범위 검색에 효율적이다. 복합 인덱스에서 **선행 컬럼**이 범위 조건에 사용되면 후행 컬럼의 인덱스 활용이 제한된다(Index Skip Scan 제외). 카디널리티와 쿼리 패턴을 함께 고려하라.
...
B+Tree 갱신은 대상 페이지 접근과 분할을 유발할 수 있고, LSM-Tree는 쓰기를 메모리·WAL에 먼저 받은 뒤 SSTable flush와 compaction으로 배치화하여 랜덤 쓰기를 줄이는 경향이 있습니다. 다만 캐시 적중과 저장 장치에 따라 실제 I/O는 달라집니다. LSM 읽기는 Bloom filter, 캐시와 compaction 상태에 따라 여러 SSTable을 확인하는 읽기 증폭을 부담할 수 있습니다. Size-Tiered와 Leveled compaction의 쓰기·읽기·공간 증폭은 설정과 데이터 분포로 측정해야 합니다.
...
**파티션 프루닝**: 파티션 키 조건을 엔진이 추론할 수 있으면 불필요한 파티션의 스캔을 줄입니다. 로컬·글로벌 인덱스의 지원과 유지 비용은 엔진별로 다릅니다. `DROP PARTITION`은 대개 행 단위 `DELETE`를 피하지만 메타데이터 잠금, 파일 정리와 복제 비용이 남을 수 있고, `DELETE` 비용은 영향받는 행·인덱스·로그 양에 따라 증가합니다.
...
**2PC**에서 각 참여자의 prepare 단계는 WAL에 prepare 레코드를 기록하여 크래시 후에도 투표 결과를 복구할 수 있게 한다. **Saga**의 보상 트랜잭션 실패 시에는 재시도 큐(dead letter queue), 수동 개입, 또는 멱등성 보장이 필요하다. **Outbox 패턴**은 비즈니스 데이터와 이벤트를 **같은 로컬 트랜잭션**으로 기록하여 WAL의 원자성을 활용 — 트랜잭션이 커밋되면 두 로컬 레코드가 함께 확정되고, 롤백되면 둘 다 취소된다. 별도의 CDC(Change Data Capture) 프로세스가 Outbox 테이블을 읽어 메시지 브로커로 발행한다.
