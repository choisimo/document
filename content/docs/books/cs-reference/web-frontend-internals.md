# Web & Frontend Internals 학습 및 기록 노트

## 1. 왜 필요한가? (Pain Point & Motivation)

웹 성능과 UI 버그는 HTML/CSS/JavaScript 문법만으로 설명되지 않는다. Browser는 HTML을 DOM으로 파싱하고, CSSOM과 합쳐 render tree를 만들며, layout, paint, composite 단계로 화면을 만든다. JavaScript는 event loop와 microtask queue 위에서 실행되고, V8은 type feedback을 기반으로 JIT 최적화와 deoptimization을 반복한다. React는 Fiber tree와 scheduler로 렌더링 작업을 쪼갠다.

이 문서는 원문의 web/frontend internals 내용을 browser engine, JavaScript runtime, React reconciliation 중심으로 재작성한다.
...
프론트엔드 data flow는 네트워크, parser, JavaScript runtime, style calculation, layout, GPU composition까지 이어진다.
...
동기 `<script>`는 HTML parser를 멈출 수 있고, microtask queue가 비워지지 않으면 `requestAnimationFrame`과 rendering 단계가 실행되지 못한다.
...
- [x] Browser rendering pipeline과 event loop를 상태 전이로 정리했다.
...
- [x] 원문 web/frontend internals 문서를 12개 섹션 템플릿으로 재작성했다.
