...
class AIChatbot {
  ...
  renderMarkdown(text) {
    ...
    // 링크 - 허용 스킴: http/https/mailto (절대), / 또는 # 시작 (상대/앵커, 동일 출처)
    // javascript: 등 위험 스킴 차단. href 속성 이스케이프로 속성 탈출 방지.
    html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, label, url) => {
      const trimmed = url.trim();
      const isAbsolute = /^https?:\/\//i.test(trimmed) || /^mailto:/i.test(trimmed);
      const isRelative = /^\/(?!\/)/.test(trimmed) || /^#/.test(trimmed);
      if (!isAbsolute && !isRelative) return label;
      const safeHref = trimmed.replace(/["'<>]/g, '');
      const attrs = isAbsolute
