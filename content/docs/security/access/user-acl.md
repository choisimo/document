...
이 문서는 GNU/Linux 계정·그룹·홈과 POSIX ACL의 예시입니다. 배포판 도구와 파일 시스템의 지원 옵션을 확인합니다. SSH/PAM 정책을 함께 변경하는 작업은 별도 해당 문서의 구문·로그인 검증도 필요합니다.

- **사전 조건**: 대상 계정의 로그인 세션과 실행 중인 프로세스, 홈 디렉토리의 마운트 지점, ACL, 확장 속성, SELinux/AppArmor 정책을 먼저 조사합니다.
- **변경 통제**: 실제 변경할 계정 DB·홈 데이터·ACL을 복구 가능하게 백업합니다. SSH/PAM 파일도 변경할 때에는 해당 설정과 콘솔 또는 별도 관리자 세션을 먼저 확보합니다.
...
- **완료 조건**: getent passwd, 소유권·ACL, 실제 대상 사용자와 비대상 사용자의 로그인·파일 접근·생성 결과를 남깁니다. sshd 설정을 바꾼 경우에는 구문 검사와 별도 세션 접속도 확인합니다.
...
새 사용자와 홈 디렉터리 생성 예시다. `alice` 계정·그룹·홈이 이미 있으면 이 블록을 반복하지 않고 기존 소유자와 데이터를 먼저 확인한다.

```bash
sudo useradd -m -U -d /home/alice -s /bin/bash alice
sudo passwd alice
