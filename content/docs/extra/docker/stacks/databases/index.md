...
- 이미지 버전, 데이터 디렉터리, UID/GID, 문자셋·타임존과 스키마 마이그레이션 순서를 고정합니다.
- 기본 계정·예시 암호·호스트 전체 공개 포트를 운영 환경에 사용하지 않습니다.
- 완료 기준은 인증 연결, 재시작 후 데이터 유지, 백업 파일 생성과 별도 환경 복원, 디스크 부족 알림 확인입니다.
