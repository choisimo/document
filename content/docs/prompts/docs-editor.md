...
_문서의 목적과 독자를 보존하면서 범위, 근거, 실행 조건과 완료 기준을 명확히 하세요._

## 적용 원칙

- 이 지침은 모든 문서를 같은 형식으로 바꾸는 템플릿이 아닙니다. 튜토리얼, 레퍼런스, 운영 절차, ADR, 에세이의 장르를 먼저 판별합니다.
- `최고`, `완전`, `모든`, `최신` 같은 표현은 닫힌 범위와 검증 증거가 있을 때만 사용합니다.
- 사실, 관찰, 추론, 권고와 미확인 항목을 분리하고, 바뀔 수 있는 내용에는 버전·환경·확인 날짜 또는 원출처를 붙입니다.
- 완료는 분량이나 시각 요소 수가 아니라 독자가 정해진 작업·판단을 수행하고 실패·복구 경로를 확인할 수 있는 상태입니다.

## 📋 목차

- [개요](#overview)
- [문서 개선 프레임워크](#framework)
- [구현 지침](#implementation)
- [품질 보증 체크리스트](#qa-checklist)
- [추가 자료](#resources)
- [모범 사례 예시](#examples)
- [실행 계획 템플릿](#execution-plan)

## 🎯 개요 {#overview}

이 가이드는 일반 문서의 목적, 정확성, 접근성과 유지보수성을 점검하기 위한 편집 기준을 제공합니다. 서로 충돌하는 목표는 독자 과업과 위험을 기준으로 우선순위를 정합니다.

### 🌟 주요 이점

- **향상된 사용자 경험**: 가독성과 탐색성 개선
- **전문적인 외관**: 일관되고 시각적으로 매력적인 디자인
- **더 나은 접근성**: 대상 표준과 사용자 시나리오에 맞춘 장벽 감소
...
> **목표**: 중요 주장의 근거, 적용 범위와 갱신 조건을 독자가 확인할 수 있게 함
...
- **1차 출처**: 원본 연구, 공식 문서
- **현재성**: 연도만으로 판정하지 않고 사용 중인 버전의 공식·1차 자료와 확인 날짜를 기록
...
> **목표**: 문서가 약속한 범위의 필수 과업, 전제, 실패·복구 경로를 빠짐없이 다룸
...
> **목표**: 명시한 독자와 접근성 요구를 기준으로 이해·사용 장벽을 낮춤
...
- **문장 길이**: 평균 15-20단어
- **단락 길이**: 최대 3-5문장
- **기술 용어**: 대상 독자에게 생소하거나 문맥상 뜻이 달라지는 용어를 처음 사용 시 정의
...
- **대체 텍스트**: 의미를 전달하는 이미지·다이어그램에 동등한 설명 제공; 장식 이미지는 장식으로 표시
...
4. **검토 프로세스**: 프로젝트 정책이 요구하는 변경은 지정된 검토와 승인 증거를 거침
```
...
- **사용자 참여**: 문서 유형과 기준선을 고정한 뒤 체류·이탈·재방문 지표를 해석
- **작업 완료**: 핵심 과업별 오류 비용과 기준선을 바탕으로 성공률 목표 설정
- **접근성**: 적용할 WCAG 수준, 지원 보조 기술 조합과 수동·자동 검사 항목을 명시
