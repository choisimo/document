...
- **입력**: `topological_sort_bfs(n, edges)`는 `0..n-1` 정점과 방향 간선 `(u, v)`를 받으며 `u`가 `v`보다 앞선다는 뜻이다. `n >= 0`과 끝점 범위는 호출 전에 확인한다. 중복 간선은 각각 indegree 증가와 감소에 반영된다.
- **출력**: DAG이면 가능한 순서 하나를 반환하며 순서는 유일하지 않을 수 있다. 사이클로 모든 정점을 처리하지 못하면 `[]`를 반환한다. `n=0`의 정상 결과도 `[]`이므로 필요한 호출자는 별도 상태를 제공해야 한다.
- **상태와 비용**: 현재 코드의 큐는 이미 `deque`와 `popleft()`를 사용한다. 진입 차수 0인 정점을 꺼내고 그 정점에서 나가는 간선을 제거하면서 다음 후보를 넣는다. 그래프 생성과 순회를 포함해 시간·공간은 `O(V+E)`다.

완료 판정은 독립 정점, 복수 유효 순서, 중복 간선, 자기 루프, 여러 정점의 사이클, 빈 그래프를 구분하는 것이다. 성공 결과가 모든 정점을 정확히 한 번 포함하고 각 간선에서 `position[u] < position[v]`인지 확인한다. 부분 순서를 전체 성공으로 반환하지 않아야 한다.
