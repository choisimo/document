...
- **현재 구현**: `sliding_window_fixed(arr, k)`는 길이 `k`인 고정 윈도우의 최대 합을 구한다. 조건 위반 시 왼쪽을 줄이는 가변 윈도우의 설명·다이어그램은 확장 개념이며 현재 함수의 동작이 아니다.
- **입력과 오류 결과**: 배열 원소는 합산·비교 가능한 유한한 수이고 `k`는 정수다. 함수는 `k <= 0` 또는 `k > len(arr)`이면 `None`을 반환하므로 빈 배열도 유효한 윈도우를 만들 수 없다.
- **출력과 비용**: 유효한 입력에서는 최대 합만 반환하며 시작·끝 인덱스는 제공하지 않는다. 초기 합 `O(k)`와 한 번의 순회로 시간 `O(n)`이다. Python의 `arr[:k]`가 길이 `k`의 임시 리스트를 만들므로 현재 구현의 최대 추가 공간은 `O(k)`다.

완료 판정은 `k=1`, `k=len(arr)`, 음수만 있는 배열, 동일 최대 합이 여럿인 경우, `k=0`, 빈 입력, `k>len(arr)`를 구분하는 것이다. 유효한 입력은 모든 길이 `k` 구간 합의 최댓값과 비교한다. 가변 윈도우에는 별도의 축소 조건과 상태 갱신을 구현한다.
