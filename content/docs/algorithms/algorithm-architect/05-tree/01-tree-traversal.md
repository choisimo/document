...
- **현재 구현**: 실행 가능한 최소 코드는 `TreeNode`와 전위 순회 `preorder(node, output)`다. 중위·후위·레벨 순회는 본문의 비교 설명과 다이어그램에 있으며 별도 함수는 포함하지 않는다.
- **입력과 출력**: 사이클과 공유 자식이 없는 이진 트리, 결과를 누적할 변경 가능한 리스트를 받는다. 전위 순서 `root-left-right`로 `node.val`을 `output`에 추가하며 반환값은 `None`이다. 빈 루트는 결과 목록을 바꾸지 않는다.
- **비용**: 노드 수 `n`, 높이 `h`에 대해 시간 `O(n)`, 재귀 스택 `O(h)`, 추가되는 결과 항목은 `O(n)`이다. 깊은 편향 트리에서는 재귀 한계를 고려해 반복형 스택을 선택한다.

완료 판정은 빈 트리, 단일 노드, 좌우 자식이 있는 트리, 편향 트리에서 전위 출력과 기존 `output` 항목 보존을 확인하는 것이다. 다른 순회 결과를 실행 결과로 제공하려면 각 함수를 추가해 별도로 검증하며, 레벨 순회에는 `deque`처럼 앞 제거가 상수 시간인 큐를 사용한다.
