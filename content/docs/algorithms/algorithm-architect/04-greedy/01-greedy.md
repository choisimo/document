...
- **현재 문제**: `select_meetings(meetings)`는 겹치지 않는 구간을 최대 개수로 선택하는 예제다. 인접하게 선택한 구간은 앞 구간의 종료가 다음 구간의 시작 이하일 때 호환된다고 정의한다. 가중 가치 최대화 등 다른 목적에는 같은 규칙을 그대로 적용하지 않는다.
- **입력과 변경 여부**: 각 항목은 비교 가능한 `(start, end)`이며 `start <= end`를 만족한다. `sorted`로 새 목록을 만들어 원본 목록 순서를 바꾸지 않는다. 종료 시각이 같으면 시작 시각이 빠른 구간을 먼저 처리해 길이 0인 구간과의 경계 순서를 일관되게 정한다.
- **출력과 정당성**: 선택한 `(start, end)` 튜플 목록을 반환한다. earliest-finish 선택이 최대 개수를 보존하는 교환 논증은 위 호환 규칙과 목적에 의존한다. 최적 해가 여럿이면 그중 하나를 반환한다.
- **비용**: 정렬 `O(n log n)`, 선택 순회 `O(n)`, 복사한 목록과 결과 공간 `O(n)`이다.

완료 판정은 빈 입력, 끝점이 맞닿는 구간, 동일 종료 시각, 완전 중첩, 길이 0 구간의 결과와 원본 목록 불변을 확인하는 것이다. 작은 입력은 모든 부분집합의 호환 여부를 확인해 선택 개수가 최적인지 비교하고, 다른 목적 함수를 적용할 때는 반례를 다시 탐색한다.
