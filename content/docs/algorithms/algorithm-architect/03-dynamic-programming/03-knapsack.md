...
- **입력**: `knapsack_01(weights, values, capacity)`에서 두 배열의 길이는 같고, 각 무게는 양의 정수이며, 용량은 0 이상의 정수다. 값은 합산·비교 가능한 수여야 한다. 현재 함수는 이러한 조건을 직접 검증하지 않으므로 호출 전에 확인한다.
- **상태**: `dp[i][w]`는 앞의 `i`개 물건을 각각 최대 한 번 선택해 용량 `w` 이내에서 얻는 최대 가치다. 빈 선택을 허용하므로 모든 가치가 음수라면 결과는 0이다.
- **출력과 비용**: 최대 가치만 반환하고 선택 목록은 복원하지 않는다. 물건 수 `n`, 용량 `C`에 대해 시간·공간은 `O(nC)`이며 숫자 용량에 의존하는 의사 다항 알고리즘이다.

완료 판정은 빈 물건 목록, 용량 0, 어떤 물건도 담지 못하는 경우, 넣기와 제외가 경쟁하는 경우, 같은 물건을 반복 사용하면 잘못 커지는 반례를 확인하는 것이다. 길이 불일치·무게 0·음수·비정수 용량은 입력 경계에서 차단한다. 선택 목록이 필요하면 별도 역추적을 구현한다.
