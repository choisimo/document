...
- **입력**: `floyd_warshall(edges, n)`는 `1..n` 정점의 방향 간선 `(u, v, weight)`를 받는다. `n >= 0`, 정점 범위, 유한한 가중치를 확인하고 무방향 간선은 양방향으로 제공한다.
- **초기화와 출력**: 길이 `n + 1`의 행·열을 가진 거리 행렬을 반환하며 0번 행·열은 사용하지 않는다. 대각선은 0으로 초기화하고 같은 방향의 중복 간선은 최소 가중치를 사용한다. 이후 더 짧은 우회 경로가 있으면 직접 간선 값도 갱신된다.
- **도달성과 음수 사이클**: 도달 불가능 경로는 Python의 `float('inf')`를 사용한다. 실행 후 `dist[v][v] < 0`이면 음수 사이클이 있으며, 그 사이클을 경유할 수 있는 쌍의 결과를 유한한 최단 거리로 해석하면 안 된다. 함수 자체는 별도의 사이클 상태를 반환하지 않는다.
- **비용**: 시간 `O(V^3)`, 거리 행렬 공간 `O(V^2)`다. 큰 정점 수에서는 공간 상한도 함께 계산한다.

완료 판정은 중복 간선, 음수 간선이 있으나 음수 사이클은 없는 그래프, 도달 불가능 쌍, 음수 자기 루프를 구분하는 것이다. 음수 사이클이 없는 작은 그래프에서 각 출발점의 Bellman–Ford 결과와 교차 확인할 수 있다.
