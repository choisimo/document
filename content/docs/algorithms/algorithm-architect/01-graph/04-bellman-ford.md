...
- **입력**: `bellman_ford(start, edges, n)`는 `1..n` 정점의 방향 간선 `(u, v, weight)`를 받는다. 정점 범위, `n >= 1`, 유효한 시작점과 유한한 가중치를 호출 전에 확인한다. 음수 가중치는 허용한다.
- **출력**: 시작점에서 도달 가능한 음수 사이클이 있으면 `None`, 없으면 길이 `n + 1`의 거리표를 반환한다. 0번 칸은 사용하지 않으며 도달 불가능 정점은 `float('inf')`다. 다른 연결 성분의 음수 사이클은 탐지 대상이 아니다.
- **처리**: 현재 구현은 `n - 1`회 완화 후 전체 간선을 한 번 더 검사한다. 갱신이 없을 때 조기 종료하는 최적화는 아직 포함하지 않는다.
- **비용**: 시간 `O(VE)`, 입력 간선을 제외한 거리표 공간 `O(V)`다. 전체 그래프의 음수 사이클을 검사하려면 초기화와 반환 계약을 별도로 설계한다.

완료 판정은 음수 간선이 있으나 사이클이 없는 경우, 도달 가능한 음수 사이클, 도달 불가능 음수 사이클을 구분하는 것이다. 거리표가 반환되면 시작점 0, 도달 불가능 값, 도달 가능한 간선의 완화 부등식을 확인한다.
