...
- **현재 함수**: `bfs(start_node, graph, target=None)`는 인접 정점 목록을 값으로 갖는 mapping과 hash 가능한 정점 라벨을 사용한다. 정점 번호가 `1..n`이어야 하는 구현은 아니다. mapping에 없는 정점은 이웃이 없는 정점으로 취급하므로 시작점과 간선 끝점의 유효성은 호출 전에 확인한다.
- **출력**: 목표를 발견하면 그 정점 라벨을 반환하고, 찾지 못하거나 `target=None`으로 순회만 수행하면 `None`을 반환한다. 방문 순서나 거리표는 반환하지 않으며, `None`은 목표 미지정 값이므로 해당 라벨 검색의 성공 여부를 구분하지 못한다.
- **방문과 종료**: 큐에 넣을 때 방문 집합에 추가해 중복 삽입을 막는다. 목표를 찾으면 조기 종료하고, 목표가 없으면 시작점에서 도달 가능한 정점만 순회한다. 방향 그래프에서는 간선 방향을 따른다.
- **비용**: 인접 리스트와 평균 상수 시간의 집합 연산을 전제로 전체 도달 범위의 시간은 `O(V_r + E_r)`, 큐와 방문 집합 공간은 `O(V_r)`다. BFS의 최단 간선 수 성질은 무가중 그래프에 적용되지만 이 함수는 거리를 계산하지 않는다.

완료 판정은 문자열 라벨, 시작점 자체가 목표인 경우, 도달 불가능 목표, 사이클, 방향 간선, 목표 없는 순회에서 반환값과 종료를 확인하는 것이다. 방문 순서·거리·경로가 필요하면 그 결과를 저장하고 반환하는 기능을 추가한 뒤 별도로 검증한다.
