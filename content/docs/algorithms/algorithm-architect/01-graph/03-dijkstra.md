...
- **입력**: `dijkstra(start, graph, n)`에서 정점은 `1..n`, `graph[u]`의 원소는 `(neighbor, weight)`다. `n >= 1`, 유효한 시작점과 간선 끝점, 모든 방문 정점의 인접 목록을 호출 전에 확인한다. 무방향 간선은 양방향으로 저장한다.
- **가중치**: 유한하고 0 이상인 가중치만 허용한다. 음수 간선은 이 구현의 계약 밖이며, 함수가 이를 직접 거부하지 않으므로 입력 단계에서 확인한다.
- **출력과 상태**: 길이 `n + 1`의 거리표를 반환하고 0번 칸은 사용하지 않는다. 시작점 거리는 0, 도달 불가능 정점은 `float('inf')`다. 힙에서 꺼낸 거리가 현재 거리보다 크면 오래된 항목으로 건너뛴다.
- **비용**: 각 완화 결과를 별도 힙 항목으로 넣는 구현의 보수적 상한은 시간 `O(V + E log(E + 1))`, 그래프를 제외한 추가 공간 `O(V + E)`다. 단순 그래프에서는 통상 `O((V + E) log V)`로 표현한다.

완료 판정은 가중치 0, 더 짧은 우회 경로, 오래된 힙 항목, 도달 불가능 정점과 병렬 간선을 포함한 작은 그래프에서 기대 거리와 일치하는 것이다. 도달 가능한 각 간선에 `dist[v] <= dist[u] + w`가 성립하는지도 확인한다.
