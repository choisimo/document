...
- **현재 함수**: `dfs_recursive(node, graph, visited=None, target=None)`는 hash 가능한 라벨을 정점으로 사용하는 인접 목록 mapping을 받는다. `1..n` 인덱스를 강제하지 않으며, mapping에 없는 정점은 이웃이 없는 정점으로 취급한다.
- **출력과 상태**: 목표를 발견하면 `True`, 찾지 못하거나 `target=None`이면 `False`를 반환한다. 방문 순서나 경로를 반환하지 않는다. 전달받은 `visited` 집합은 제자리에서 갱신하며, 목표를 찾으면 남은 분기는 순회하지 않는다.
- **그래프 계약**: 무방향 간선은 양쪽 인접 목록에 넣고, 방향 간선은 주어진 방향으로만 순회한다. 사이클은 방문 집합으로 차단한다. 전체 도달 집합이 필요하면 목표를 지정하지 않고 빈 `visited` 집합을 전달한다.
- **비용과 깊이**: 인접 리스트와 평균 상수 시간의 집합 연산을 전제로 시간은 `O(V_r + E_r)`, 방문 집합과 최악의 재귀 스택은 `O(V_r)`다. 깊은 경로는 Python 재귀 한도를 넘을 수 있으므로 입력 상한에 맞춰 반복형 스택을 선택한다.

완료 판정은 목표 발견·부재, 사이클, 문자열 라벨, 목표 없는 전체 도달 집합, 조기 종료 후 `visited`의 범위를 확인하는 것이다. 모든 연결 성분을 순회하려면 아직 방문하지 않은 정점에서 반복 호출하며, 반환 `False`를 순회 실패로 해석하지 않는다.
