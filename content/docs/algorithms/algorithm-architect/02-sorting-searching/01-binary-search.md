...
- **입력**: `binary_search(arr, target)`는 비교 연산과 같은 기준으로 비내림차순 정렬된 배열을 받는다. 정렬 여부를 함수가 검사하거나 수정하지 않는다. 사용자 정의 객체는 정렬과 탐색에 일관된 비교 기준을 제공해야 한다.
- **출력**: 일치하는 원소의 인덱스 하나를 반환하며, 빈 입력이나 일치하는 원소가 없으면 `-1`이다. 중복 값의 첫 번째·마지막 인덱스를 보장하지 않는다.
- **불변식과 비용**: 정답이 있다면 닫힌 탐색 구간 `[left, right]`에 남도록 경계를 갱신한다. 배열의 임의 접근을 상수 시간으로 볼 때 시간 `O(log n)`, 추가 공간 `O(1)`이다.

완료 판정은 빈 배열, 단일 원소, 양끝의 목표, 중복 값, 목표 부재를 확인하는 것이다. 반환 인덱스가 유효하면 `arr[index] == target`, `-1`이면 입력에 목표가 없음을 확인한다. 첫 위치나 삽입 위치가 필요하면 반환 계약과 루프 경계를 별도로 변경한다.
