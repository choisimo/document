...
- **입력**: `merge_sort(arr)`는 일관된 비교 순서를 가진 원소의 리스트를 받는다. 보조 함수 `merge(left, right)`에는 이미 같은 기준으로 정렬된 두 리스트를 전달한다.
- **반환과 안정성**: 입력 목록을 제자리에서 정렬하지 않는다. 길이가 2 이상이면 슬라이스와 병합 결과를 새로 만들고, 길이가 0 또는 1이면 입력 객체를 그대로 반환한다. 동률일 때 `left[i] <= right[j]` 분기로 왼쪽 원소를 먼저 선택해 같은 키의 원래 순서를 유지한다.
- **비용**: 시간 `O(n log n)`, 최대 동시 보조 공간 `O(n)`, 재귀 깊이 `O(log n)`이다. Python 슬라이스와 병합 결과의 누적 할당량은 최대 동시 메모리와 구분해 측정한다.

완료 판정은 빈 입력, 단일 원소, 중복 키, 정렬·역정렬 입력에서 순서와 원소 다중집합이 보존되는지 확인하는 것이다. 중복 키에 서로 다른 식별자를 붙여 상대 순서를 검사하고, 입력 목록 불변과 base case 객체 공유도 확인한다.
