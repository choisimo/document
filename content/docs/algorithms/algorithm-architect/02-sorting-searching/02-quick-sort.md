...
- **입력과 비교**: `quick_sort(arr)`의 원소는 `<`, `==`, `>`가 일관된 전체 순서를 이뤄야 한다. 예를 들어 NaN처럼 어느 비교에도 속하지 않는 값은 이 분할 구현의 계약 밖이다.
- **반환과 변경 여부**: 함수는 입력을 제자리에서 정렬하지 않는다. 길이가 2 이상이면 새 분할 리스트와 병합 결과를 만들지만, 길이가 0 또는 1인 base case는 입력 리스트 객체를 그대로 반환한다.
- **피벗과 비용**: 가운데 위치의 값을 피벗으로 사용한다. 균형 있는 분할에서는 시간 `O(n log n)`이지만 최악 `O(n^2)`을 방지하는 보장은 없다. 새 리스트와 재귀 호출의 할당도 필요하며, 분할이 심하게 치우치면 재귀 깊이와 메모리 사용이 커진다.

완료 판정은 빈 입력, 단일 원소, 같은 값만 있는 입력, 정렬·역정렬 입력에서 결과의 비내림차순과 원소 다중집합 보존을 확인하는 것이다. 입력 목록이 바뀌지 않는지와 base case의 객체 공유도 구분한다. 성능 보장이 필요하면 실제 입력 분포와 최악 분할에 대한 전략을 별도로 검토한다.
