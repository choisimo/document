...
여기서 “모든 템플릿”은 아래 문서 색인에 열거된 파일 집합을 뜻한다. 각 페이지는 실행 가능한 범용 해답이 아니라 특정 코드 골격의 해설이며, 입력 전제와 실패 상태는 페이지별 적용 계약을 우선한다. Playwright 명령을 실행했다는 사실만으로 검증 완료가 되지는 않는다. 완료는 대상 페이지 목록, 프로세스 종료 상태, Mermaid·heading 오류 0건, 생성된 summary 경로와 실패 screenshot을 함께 기록해 판정한다.
