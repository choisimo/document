...
- **현재 문제**: `combination_sum(candidates, target)`는 각 후보 인덱스를 최대 한 번만 사용해 합이 목표가 되는 조합을 찾는다. 재귀 호출에 `i + 1`을 넘기므로 같은 인덱스를 무제한 재사용하는 구현이 아니다.
- **입력과 가지치기**: 이 예제는 양의 정수 후보와 0 이상의 정수 목표를 전제로 한다. 정렬 후 `value > remaining`이면 이후 후보도 사용할 수 없다는 가지치기를 적용한다. 다른 부호·수치 정책에는 가지치기와 종료 계약을 다시 정한다.
- **변경과 출력**: `candidates.sort()`로 입력 목록을 제자리에서 정렬하고, 결과에는 각 `path`의 복사본을 저장한다. 목표 0은 빈 조합 `[[]]`을 반환한다. 서로 다른 인덱스에 같은 값이 있으면 값 기준으로 같은 조합이 중복될 수 있으며 중복 제거는 구현하지 않는다.
- **비용**: 최악 탐색 공간은 후보 수에 대해 지수적으로 커지고 결과 복사 비용도 든다. 각 인덱스를 한 번만 쓰므로 재귀 깊이는 후보 수 이하이며, 모든 조합의 복사를 포함하면 최악 시간·결과 크기를 `O(n 2^n)`으로 제한할 수 있다.

완료 판정은 정상 조합, 목표 0, 해 없음, 후보 하나를 재사용해야만 가능한 반례, 중복 후보, 호출 후 정렬과 결과 복사를 확인하는 것이다. 무제한 재사용 변형은 재귀 인덱스를 `i`로 바꾸고 양수·종료 조건을 재검토하며, 값 중복 제거가 필요하면 같은 깊이의 동일 후보를 건너뛰는 규칙을 추가한다.
