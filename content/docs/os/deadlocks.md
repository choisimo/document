...
교착 상태는 작업 집합이 자신 또는 서로가 유발할 사건·자원 해제를 기다려 외부 개입 없이는 진행할 수 없는 상태다. 영향은 해당 작업 집합에 국한되거나 공유 자원 경로로 확대될 수 있으며 lock, 장치와 트랜잭션 등의 관리에서 나타난다.

## 문서 범위와 검증 기준

- **범위**: 재사용 가능 자원과 고정된 요청·할당 상태를 다루는 교과서 모델입니다. 데이터베이스 lock manager, 커널, 분산 트랜잭션의 실제 정책은 자원 의미와 버전에 따라 다릅니다.
- **전제**: Coffman 네 조건과 그래프 판정은 자원 인스턴스 수, 선점 가능성과 요청 모델을 명시했을 때 적용합니다. 아래 lock 순서 의사 코드는 interleaving에 따라 교착이 가능하며 매 실행에서 반드시 교착되는 것은 아닙니다.
...
아래는 R1·R2가 각각 단일 인스턴스이고 다른 경로에서 해제·선점할 수 없는 모델이다. P1·P2가 서로 보유한 자원의 해제를 기다리면 진행할 수 없다.
...
- Unsafe state는 최대 요구량 모델에서 safe sequence를 보장할 수 없는 상태다. 현재 deadlocked일 수도 있고 아직 진행 가능하지만 이후 교착될 수 있는 상태일 수도 있다.

## 8. 가장 작은 예제 (Minimal Viable Example)

아래는 두 mutex를 서로 다른 순서로 획득하는 의사 코드다. A를 얻은 thread_a와 B를 얻은 thread_b가 각 두 번째 lock을 기다리는 interleaving에서 cycle이 생기며, 한 thread가 먼저 완료하면 교착하지 않는다.
...
Coffman 네 조건은 이 자원 모델에서 교착의 필요 조건이며, 적용 범위·인스턴스 수를 확인해야 한다. 전략은 조건을 미리 깨뜨리거나 최대 요구량 아래 안전 상태만 허용하거나 발생을 탐지하고 복구하는 것이다.
