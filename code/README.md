...
│           ├── python/        # Python 구현 (Reference Counting)
│           ├── javascript/    # JavaScript 구현 (V8 최적화)
│           ├── java/          # Java 구현 (JVM/GC)
│           └── rust/          # Rust 구현 (Ownership)
├── simulator/                 # 🎮 인터랙티브 시뮬레이터 (알고리즘별 조직)
│   ├── tree/
│   │   ├── binary-search-tree/{python,javascript,java}/
│   │   └── avl-tree/python/
│   ├── hash/hash-table/{python,javascript,java}/
│   ├── heap/{python,javascript,java}/
│   ├── linear/
│   │   ├── linked-list/python/
│   │   └── stack-queue/python/
│   ├── search/binary-search/python/
│   ├── sort/{python,javascript,java}/
│   ├── graph/python/
│   ├── javascript/            # 웹 시뮬레이터 공통 파일
│   ├── python/                # Python 공통 유틸
│   └── shared/                # 공용 테스트 케이스
├── competitive-programming/   # 코딩 테스트 문제 풀이
│   ├── leetcode/
│   └── basics/               # 기초 문제 모음
│       ├── bst-insertion/java/
│       ├── bst-deletion/java/
│       └── hash-table/java/
├── algorithmic-thinking/     # 알고리즘 사고 문제
├── theory/                    # 이론 정리 (마크다운)
├── c-lang/                    # 시스템 프로그래밍 (C)
├── LICENSE
├── README.md
...
```bash
# 자료구조
cd simulator/tree/binary-search-tree/python && python bst_simulator.py
cd simulator/tree/avl-tree/python && python avl_simulator.py
cd simulator/hash/hash-table/python && python hashtable_simulator.py
cd simulator/heap/python && python heap_simulator.py
cd simulator/linear/linked-list/python && python linkedlist_simulator.py
cd simulator/linear/stack-queue/python && python stack_queue_simulator.py

# 알고리즘
cd simulator/search/binary-search/python && python binary_search_simulator.py
cd simulator/sort/python && python sorting_simulator.py
cd simulator/graph/python && python graph_simulator.py
...
```bash
# Tree
cd simulator/tree/binary-search-tree/java
javac BSTSimulator.java && java BSTSimulator

# Hash
cd simulator/hash/hash-table/java
javac HashTableSimulator.java && java HashTableSimulator

# Heap
cd simulator/heap/java
javac HeapSimulator.java && java HeapSimulator

# Sort
cd simulator/sort/java
javac SortingSimulator.java && java SortingSimulator
```
