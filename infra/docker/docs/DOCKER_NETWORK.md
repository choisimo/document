...
Docker는 컨테이너 간 통신 및 외부 네트워크 연결을 위해 여러 네트워크 설정 방법을 지원한다. 이 문서는 Docker 네트워크의 기본 개념과 설정 방법을 정리한다.
...
Docker는 다양한 사용 사례에 맞는 여러 네트워크 드라이버를 포함한다[3]:
...
**ipvlan**: IPv4 및 IPv6 주소 지정을 세밀하게 제어할 수 있다[3].
