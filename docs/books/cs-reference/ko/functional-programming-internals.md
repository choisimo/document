...
핵심 설계 가치는 **불가능한 상태를 표현 불가능하게 만드는 것**이다. `String | null` 대신 `Maybe String`을 사용하면 null 처리를 타입 시스템이 강제한다.
...
`ZIO[R, E, A]`는 "환경 R을 필요로 하고, E로 실패하거나 A로 성공하는 계산"을 표현하는 타입이다. 테스트에서 R에 테스트용 구현을 주입하면 실제 DB/네트워크 없이 비즈니스 로직을 검증할 수 있다. Mock 프레임워크는 런타임에 동작하지만, 환경 타입은 컴파일 타임에 필요한 의존성을 보장한다. `ReaderTaskEither<R, E, A>`는 `ZIO[R, E, A]`와 거의 동일한 구조이다.
