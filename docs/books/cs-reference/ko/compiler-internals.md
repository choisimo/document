...
**일반적인 최적화 순서 원칙**:
1. **인라이닝 먼저**: 함수 호출을 제거하고 호출자 컨텍스트에서 추가 최적화 기회를 열어준다.
...
Hindley-Milner(HM) 타입 추론은 **통합(unification)** 알고리즘을 사용한다. `let x = vec![1, 2, 3]`에서: (1) `x`에 타입 변수 `?T` 할당, (2) `vec!` 매크로가 `Vec<U>`를 반환, (3) 리터럴 `1, 2, 3`이 정수이므로 `U = i32`로 통합, (4) `?T = Vec<i32>`로 결정. Rust가 함수 경계를 넘지 않는 이유: (1) 전역 타입 추론은 O(n³) 이상으로 컴파일 시간이 폭발, (2) 함수 하나의 변경이 전체 타입에 영향을 주어 오류 메시지가 모호해진다(Haskell의 유명한 문제), (3) 증분 컴파일이 어려워진다. 부분 추론(함수 본문 내부만)으로 제한하면 이 문제들이 해결된다.
